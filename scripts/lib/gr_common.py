...
def test_doc_number(key: str, token=None, band: int = 9000) -> str:
    ...
    import random
    tok = token or login()
    st, txt = http("GET", f"/doc-number-policy?key={key}", tok)
    sample, pol = "", {}
    if st == 200:
        try:
            pol = json.loads(txt) or {}
            sample = pol.get("contoh") or ""
        except Exception:  # noqa: BLE001
            sample = ""
    # 2026-09-12: di basis data go-live SEMUA seri bermode OTOMATIS (tidak ada config
    # manual warisan demo) → nomor ketikan ditolak 400. Gate memerlukan nomor yang
    # diketahui di muka, jadi mode diubah ke MANUAL sementara dan DIKEMBALIKAN saat
    # proses gate selesai (atexit) — setelan owner tidak berubah permanen.
    if pol.get("mode") == "auto" and pol.get("policy_enforced", True):
        _ensure_manual_mode_for_gate(key, tok, was_custom=bool(pol.get("mode_is_custom")))
...
_GATE_MODE_SWITCHED: dict = {}


def _ensure_manual_mode_for_gate(key: str, token, *, was_custom: bool) -> None:
    import atexit
    if key in _GATE_MODE_SWITCHED:
        return
    st, _ = http("PUT", "/admin/doc-numbering", token, body={"key": key, "mode": "manual", "active": True})
    if st != 200:
        return
    _GATE_MODE_SWITCHED[key] = {"token": token, "restore": "auto" if was_custom else None}
    if len(_GATE_MODE_SWITCHED) == 1:
        atexit.register(restore_gate_doc_modes)


def restore_gate_doc_modes() -> None:
    """Kembalikan mode seri nomor yang diubah gate (dipanggil otomatis saat proses selesai)."""
    for key, info in list(_GATE_MODE_SWITCHED.items()):
        tok = info["token"] or login()
        http("PUT", "/admin/doc-numbering", tok, body={"key": key, "mode": "auto", "active": True})
        if info["restore"] is None:
            # semula TIDAK ada setelan kustom → hapus field mode supaya kembali ke bawaan registry
            try:
                from pymongo import MongoClient
                env = {}
                ...
                MongoClient(env["MONGO_URL"])[env["DB_NAME"]].doc_number_configs.update_one(
                    {"key": key}, {"$unset": {"mode": ""}})
            except Exception:  # noqa: BLE001
                pass
        _GATE_MODE_SWITCHED.pop(key, None)

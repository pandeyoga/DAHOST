...
def main() -> int:  # noqa: C901
    ...
    try:
        ...
        db.marketing_platform_accounts.update_one(
            {"id": acc_id},
            {"$unset": {"ar_account_code": "", "coa_receivable_code": "", "coa_revenue_code": ""}})
        ...
        if st == 400 and "ar_account_code" in msg and "coa_revenue_code" in msg:
        ...
        rev = db.rahaza_coa_accounts.find_one(
            {"type": "REVENUE", "name": {"$regex": "Shopee", "$options": "i"}, "is_group": {"$ne": True}, "active": True},
            sort=[("code", 1)])
        ...
        if recv_code in codes and rev_code in codes and cash_code not in codes:
            ok("B4", f"jurnal dana dilepas: Dr piutang toko / Cr pendapatan MILIK TOKO "
                     f"({recv_code} / {rev_code}); bank {cash_code} TIDAK disentuh (belum ditarik)")
        else:
            bad("B4", "jurnal dana dilepas tidak memakai akun saldo platform toko", det(sorted(codes)))
        ...
        for f in sorted(rdir.glob("marketing_*.py")):
            if f.name in ("marketing_settlements.py", "marketing_withdrawals.py"):
                continue
        ...
        if not offenders:
            ok("C1", f"0 dari {len(list(rdir.glob('marketing_*.py'))) - 2} berkas "
                     f"routes/marketing_* memanggil pembuat jurnal — hanya dana dilepas & penarikan")
        else:
        ...
        # ══ F. TAHAP 2 — PENARIKAN SALDO KE BANK ══════════════════════════════
        head("F — SALDO PLATFORM → PENARIKAN KE BANK (Dr Bank / Cr Piutang Toko)")

        st, r = call("GET", "/api/marketing/settlements/platform-balance", admin)
        row = next((x for x in (r.get("data") or []) if x.get("account_id") == acc_id), {}) if st == 200 else {}
        if st == 200 and abs(float(row.get("balance") or 0) - 15_000_000) < 1:
            ok("F1", "saldo platform toko = Σ dana dilepas (8jt + 7jt) sebelum ada penarikan",
               det({k: row.get(k) for k in ("released_total", "withdrawn_total", "balance", "gl_balance")}))
        else:
            bad("F1", f"saldo platform tidak sesuai (HTTP {st})", det(row))

        wd_base = {"account_id": acc_id, "withdrawal_date": "2026-08-22", "notes": TAG}
        st, r = call("POST", "/api/marketing/withdrawals", fin_tok,
                     {**wd_base, "amount": 20_000_000, "reference": f"{TAG}-WDX"})
        if st == 400 and "melebihi saldo" in str(r.get("detail", "")):
            ok("F2", "penarikan melebihi saldo platform DITOLAK (saldo tidak boleh minus)")
        else:
            bad("F2", f"penarikan melebihi saldo tidak ditolak (HTTP {st})", det(r))

        st, r = call("POST", "/api/marketing/withdrawals", mkt_tok,
                     {**wd_base, "amount": 5_000_000, "reference": f"{TAG}-WDM"})
        if st == 403:
            ok("F3", "peran Marketing DITOLAK mencatat penarikan (403)")
        else:
            bad("F3", f"peran Marketing bisa mencatat penarikan (HTTP {st})", det(r))

        st, r = call("POST", "/api/marketing/withdrawals", fin_tok,
                     {**wd_base, "amount": 9_000_000, "reference": f"{TAG}-WD1"})
        wid = (r.get("data") or {}).get("id") if st == 200 else None
        if wid:
            ok("F4", "Finance mencatat penarikan 9jt dari saldo 15jt")
        else:
            bad("F4", f"penarikan sah gagal dicatat (HTTP {st})", det(r))
            return 1

        st, r = call("POST", f"/api/marketing/withdrawals/{wid}/journal", fin_tok)
        je = db.rahaza_journal_entries.find_one({"id": r.get("je_id")}) if st == 200 else None
        lines = {(ln["account_code"], "D" if ln["debit"] else "C") for ln in (je or {}).get("lines", [])}
        if je and (cash_code, "D") in lines and (recv_code, "C") in lines and je.get("status") == "draft":
            ok("F5", f"jurnal penarikan: Dr {cash_code} (bank toko) / Cr {recv_code} (piutang toko), DRAFT")
        else:
            bad("F5", f"jurnal penarikan salah (HTTP {st})", det(sorted(lines)))

        st, r = call("GET", "/api/marketing/settlements/platform-balance", admin)
        row = next((x for x in (r.get("data") or []) if x.get("account_id") == acc_id), {}) if st == 200 else {}
        if abs(float(row.get("balance") or 0) - 6_000_000) < 1:
            ok("F6", "saldo platform turun jadi 6jt setelah penarikan 9jt (= yang masih bisa dicairkan)")
        else:
            bad("F6", "saldo platform tidak berkurang setelah penarikan", det(row))

        st, r = call("DELETE", f"/api/marketing/settlements/{sids[1]}", fin_tok)
        if st == 400 and "ditarik" in str(r.get("detail", "")):
            ok("F7", "dana dilepas yang sudah ikut ditarik tidak bisa dihapus begitu saja")
        else:
            bad("F7", f"dana dilepas yang sudah ditarik bisa dihapus (HTTP {st})", det(r))
        ...
        db.marketing_platform_withdrawals.delete_many({"notes": TAG})
        if acc_id:

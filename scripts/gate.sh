...
run_gate "KEAMANAN — endpoint tulis tanpa gerbang peran tidak boleh naik (audit_authz, T-01)" \
         "python3 scripts/audit_authz.py --gate"
run_gate "UANG — JE aktif per sumber harus tunggal (T-10)" \
         "python3 scripts/find_duplicate_active_je.py"
run_gate "SSOT — koleksi yang dibaca tanpa penulis tidak boleh naik (check_collection_writers, T-03/T-17)" \
         "python3 scripts/check_collection_writers.py --gate"
run_gate "KINERJA — to_list(None) di routes tidak boleh naik (check_unbounded_queries, T-21)" \
         "python3 scripts/check_unbounded_queries.py --gate"
...
if [ $AUTH_READY -eq 1 ]; then
  ...
  run_gate "DATA/UANG — Marketing: impor KPI Seller Center + assign toko + scorecard (INV-KPIIMPOR)" \
           "python3 tests/legacy/test_core_f7_kpi_impor.py"
  ...
  run_gate "DATA/UANG — Marketing: status pengiriman (Ekspor B/C) + pemulihan impor (INV-MKTFULFILL)" \
           "python3 tests/legacy/test_core_f3_fulfillment.py"
  ...
  run_gate "DATA/KEWENANGAN — Marketing: assign toko · ingat pemetaan · scorecard kreator (INV-MKTOPS)" \
           "python3 tests/legacy/test_core_f8_assign_ingat_scorecard.py"
  ...
  run_gate "UANG/ARAH — Marketing: omzet bruto vs setelah retur (INV-RETUR)" \
           "python3 tests/legacy/test_core_returns_visibility.py"
  ...
  run_gate "KEAMANAN/DATA — Marketing: lingkup toko per pemakai + jejak perubahan (INV-F6RBAC)" \
           "python3 tests/legacy/test_core_f6_rbac_scope.py"
  ...
  run_gate "BISA DIPAKAI — Marketing: layar daftar (tabel · cari · unduh) + field tak disembunyikan (INV-F10)" \
           "python3 tests/legacy/test_core_f10_layar_daftar.py"
  ...
  run_gate "DATA/UANG — Marketing: pratinjau impor per baris = kenyataan (INV-F11)" \
           "python3 tests/legacy/test_core_f11_pratinjau_impor.py"
  ...
  run_gate "DATA/UANG — Marketing: berkas ekspor tidak boleh masuk toko yang salah (INV-F12)" \
           "python3 tests/legacy/test_core_f12_sidik_toko.py"
  ...
  # ── INV-F46 (2026-09-12) — BOM MEMAKAI POTONGAN, HPP DARI KAIN YANG DIPOTONG ──
  # Keputusan owner: BOM tidak lagi menyebut kg kain (menyesatkan). Bahan utama =
  # 1 pcs potongan CUT-<MODEL>-<WARNA>-<UKURAN>; biaya potongan = nilai kain roll yang
  # benar-benar dipotong ÷ pcs jadi (roll 150 yard Rp 3 jt → 150 pcs → Rp 20.000).
  run_gate "UANG — BOM potongan & HPP dari kain yang dipotong (INV-F46)" \
           "python3 scripts/verify_bom_potongan_hpp.py"

  # ── INV-F47 (2026-09-12) — BAGAN AKUN TUNGGAL 4 DIGIT, SINKRON DENGAN AUTO GL ──
  run_gate "UANG — bagan akun tunggal 4 digit & sinkron auto GL/channel/sub-ledger (INV-F47)" \
           "python3 scripts/verify_coa_unified.py"

  # ── INV-F48 (2026-09-12) — LAPORAN 12 BULAN · NERACA LAJUR · SALDO AWAL LAYAR · ARUS KAS PER AKUN ──
  run_gate "UANG — laporan 12 bulan, neraca lajur, saldo awal via layar, arus kas OPR/INV/PND (INV-F48)" \
           "python3 scripts/verify_laporan_keuangan_ext.py"

  # ── INV-F49 (2026-09-12) — MASTER KEUANGAN ↔ BAGAN AKUN SINKRON (rekening, vendor CMT, toko, klien maklon) ──
  run_gate "UANG — rekening/vendor CMT/toko/klien maklon sinkron dgn bagan akun, tanpa GL dobel (INV-F49)" \
           "python3 scripts/verify_finance_sync.py"

  # ── INV-M50 (2026-09-12) — MASTER PRODUK LINTAS PORTAL: varian SSOT = BOM ∪ FG, style RnD per model, karyawan↔user ──
  run_gate "MASTER — varian SSOT ∪ BOM ∪ FG, style RnD tiap model, karyawan↔user, kategori/satuan material (INV-M50)" \
           "python3 scripts/verify_master_sync.py"
  ...
  for g in "state machine jurnal" "nomor dokumen kembar" "batas nilai AR/AP" \
           ...
           "Impor pintar punya pintu di layar (INV-F45)" \
           "BOM potongan & HPP dari kain yang dipotong (INV-F46)" \
           "Bagan akun tunggal 4 digit (INV-F47)" \
           "Laporan 12 bulan, neraca lajur, saldo awal, arus kas per akun (INV-F48)" \
           "Master keuangan sinkron bagan akun (INV-F49)" \
           "Master produk lintas portal sinkron (INV-M50)"; do
...
run_gate "LAYAR — UANG/STOK di luar Marketing bisa dipakai & dibawa (INV-F13)" \
         "python3 tests/legacy/test_core_f13_layar_uang_bisa_dibawa.py"
...
run_gate "DATA/UANG — Form wajib memakai Master, bukan ketikan (INV-F14)" \
         "python3 tests/legacy/test_core_f14_form_pakai_master.py"
...
run_gate "LAYAR — Kartu punya latar, tulisan terbaca, token tidak berbohong (INV-F15)" \
         "python3 tests/legacy/test_core_f15_kartu_terbaca.py"

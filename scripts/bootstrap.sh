...
# GO-LIVE (2026-09-17): data = DATA KLIEN NYATA (seed/DA_SEED_GOLIVE.archive.gz), bukan demo.
ensure_env SEED_DEFAULT_LOCATIONS "false"
ensure_env ALLOW_DEMO_SEED "false"
ensure_env MASTER_IMPORT_INITIAL_PASSWORD "Dewi@123"
ensure_env ALLOW_SESSION_TOKEN_IN_QUERY "1"
...
# ── GO-LIVE (2026-09-17): SEED = SNAPSHOT DATA KLIEN, BUKAN DEMO ─────────────
# seed/DA_SEED_GOLIVE.archive.gz = backup VPS klien + BOM_AKSESORIS sudah diterapkan.
# Bila arsip ada, restore-lah itu (saat DB kosong, atau --reseed) dan JANGAN
# jalankan seeder demo di bawah (ALLOW_DEMO_SEED=false → 403 juga).
if [ "$NEED_SEED" = "1" ] && [ -f "$APP/seed/DA_SEED_GOLIVE.archive.gz" ]; then
  c "  → seed data klien (snapshot VPS + BOM aksesoris)"
  if [ "$RESEED" = "1" ]; then SEED_FLAG="--force"; else SEED_FLAG=""; fi
  if bash "$APP/scripts/seed_golive_restore.sh" $SEED_FLAG 2>&1 | sed 's/^/    /'; then
    ok "seed data klien OK"
  else
    err "seed data klien GAGAL (bash scripts/seed_golive_restore.sh --force untuk mengulang)"
  fi
  NEED_SEED=0
  for i in $(seq 1 30); do curl -sf http://localhost:8001/api/health >/dev/null 2>&1 && break; sleep 2; done
  TOKEN=$(curl -s -X POST http://localhost:8001/api/auth/login -H "Content-Type: application/json" \
    -d '{"email":"admin@garment.com","password":"Admin@123"}' | python3 -c "import sys,json;print(json.load(sys.stdin).get('token',''))" 2>/dev/null)
fi
...
if [ "$HEALTHY" = "1" ]; then
  ...
  if [ -f "$APP/seed/DA_SEED_GOLIVE.archive.gz" ]; then
    printf "    (data klien: akun peran demo hr/finance/... TIDAK ada; %s user nyata di koleksi users)\n" \
      "$(curl -s http://localhost:8001/api/rahaza/users -H "Authorization: Bearer $TOKEN" | python3 -c "import sys,json;d=json.load(sys.stdin);print(d.get('total',len(d)) if isinstance(d,dict) else len(d))" 2>/dev/null || echo '?')"
  else
...
echo "  Login: admin@garment.com / Admin@123  |  data klien nyata (seed/DA_SEED_GOLIVE.archive.gz) — akun lain: users nyata, sandi awal Dewi@123"

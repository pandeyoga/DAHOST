...
def rows_for(sheet: str) -> list[list]:
    if sheet == "01_LOKASI":
        return [[f"{P}-GD01", "Gudang Utama", "gudang", "", "ya", "fg"],
    ...
    if sheet == "11_VENDOR_CMT":
        return [[f"{P}-CMT01", "CMT Pak Aan", "Aan", "081300000001", "Cimahi", 3000, 8500,
                 "spesialis kaos"],
                [f"{P}-CMT02", "CMT Bu Iis", "Iis", "081300000002", "Bandung", 1500, 12000,
                 "spesialis kemeja"]]
    ...
    if sheet == "17_USER":
        return [["dewi.contoh@dewiaditya.id", "Dewi Lestari", f"{P}-E002", "hr", "active",
                 "sandi awal dibuat sistem, wajib diganti saat login pertama"]]
    if sheet == "18_TUNJANGAN":
        return [[f"{P}-TJ-JAB", "Tunjangan Jabatan", 100_000, "fixed", "ya", "karyawan", "",
                 f"{P}-E002", "masuk dasar iuran BPJS"],
                [f"{P}-INS-MKN", "Insentif Makan", 10_000, "per_day_attendance", "tidak", "semua",
                 "", "", "Rp 10.000 × hari hadir"]]
    return []
...
def build(dest: Path) -> None:
    ...
    for line in [
        ...
        "· 16_LIVEHOST wajib menunjuk kode_karyawan — kode internal, BUKAN NIK KTP.",

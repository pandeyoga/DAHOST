...
async def main() -> int:  # noqa: C901
    ...
    COLLS = ["rahaza_locations", "rahaza_employees", "rahaza_payroll_profiles",
             ...
             "marketing_kol_creators", "marketing_livehosts", "users", "da_payroll_allowances"]

    # Hitung/bersihkan HANYA dokumen uji ber-TAG — bukan semua `import_batch master_template_*`,
    # karena di lingkungan produksi koleksi yang sama berisi data nyata hasil impor owner.
    TAG_FIELDS = ("code", "employee_code", "sku", "account_code", "creator_code", "email",
                  "notes", "color_code")

    def tag_q() -> dict:
        return {"$or": [{f: {"$regex": TAG}} for f in TAG_FIELDS]}

    async def tagged_employee_ids() -> list:
        return [e["id"] async for e in db.rahaza_employees.find(
            {"employee_code": {"$regex": TAG}}, {"_id": 0, "id": 1})]

    async def count_tag() -> int:
        n = 0
        emp_ids = await tagged_employee_ids()
        for c in COLLS:
            if c == "rahaza_payroll_profiles":
                n += await db[c].count_documents({"employee_id": {"$in": emp_ids}})
            else:
                n += await db[c].count_documents(tag_q())
        return n

    async def cleanup():
        emp_ids = await tagged_employee_ids()
        await db.rahaza_payroll_profiles.delete_many({"employee_id": {"$in": emp_ids}})
        for c in COLLS:
            if c != "rahaza_payroll_profiles":
                await db[c].delete_many(tag_q())

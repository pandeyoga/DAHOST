...
def main():
    ...
    try:
        ...
        # Warna diambil dari MASTER yang ada (bukan 'NVY'/'HTM' demo) supaya gate
        # tidak melahirkan warna baru di palet nyata dan uji nama-vs-kode adil.
        _, palette = req('GET', '/api/dewi/rnd/color-options', None, tok)
        palette = [c for c in (palette if isinstance(palette, list) else palette.get('items') or [])
                   if c.get('code') and c.get('name')]
        if len(palette) < 2:
            raise RuntimeError('master warna < 2 — gate tidak bisa mengukur varian kembar')
        c1, c2 = palette[0], palette[1]
        _, bulk = req('POST', '/api/dewi/rnd/variants/bulk', {
            'style_id': sid_, 'style_code': scode,
            'colors': [{'code': c1['code']}, {'code': c2['code']}], 'sizes': ['S', 'M'],
        }, tok)
        ...
        dup_code, dup = req('POST', '/api/dewi/rnd/variants/bulk', {
            'style_id': sid_, 'style_code': scode,
            'colors': [{'code': c1['code']}], 'sizes': ['S'],
        }, tok)
        dup2_code, _ = req('POST', '/api/dewi/rnd/variants', {
            'style_id': sid_, 'style_code': scode, 'color': c1['name'], 'sizes': [{'size': 'M'}],
        }, tok)

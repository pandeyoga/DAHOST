...
def build(dest: Path) -> None:
    ...
    for line in [
        ...
        "5. Kode (kode/kode_karyawan/sku/kode_akun) adalah KUNCI: impor ulang dengan kode sama = MEMPERBARUI,",
        ...
        "",
        "KESALAHAN YANG PALING SERING TERJADI (pelajaran dari pengisian sebelumnya):",
        "a. JANGAN mengubah NAMA SHEET (jangan tambah ✅, 'done', dsb.).",
        "b. Tanda # HANYA untuk baris contoh. Kode Anda TIDAK boleh diawali # ('#HTM' salah → 'HTM').",
        "c. Kolom kode_* diisi KODE, bukan nama: kode_warna = 'NVY' (bukan 'Navy'), kode_model = 'DA-4301' (bukan 'Dress').",
        "d. Isi SETIAP baris lengkap — jangan mengosongkan kode_model/nama pada baris lanjutan (tidak ada 'merge cell').",
        "e. Kode ukuran tanpa spasi: 'ALLSIZE' bukan 'ALL SIZE'.",
        "f. Nomor telepon diketik sebagai TEKS (awali dengan tanda ' di Excel) agar angka 0 di depan tidak hilang.",
        "g. 10_BOM: satuan sedimensi dikonversi (91,44 cm = 1 yard); cm → kg hanya dengan GSM & lebar bahan.",
        "   kode_warna opsional: kosong = umum, terisi = khusus warna. Jangan jumlahkan pilihan warna.",
        "   Qty >5 kg atau >10 yard per pcs ditolak; >3 kain sejenis dan BOM tanpa aksesoris diperingatkan.",
        "   BOM manual/hasil edit tidak ditimpa importir. Kode karyawan BUKAN NIK KTP 16 digit.",
        "h. 14_KATALOG_JUAL: harga_jual = harga tayang; harga_coret = harga SEBELUM diskon (lebih besar).",
        "i. isi_per_kemasan = ANGKA saja (144), bukan '1 karung 60kg'.",
        "",

...
async def run():
    ...
    # INV-JL-2 (T-09): setiap JE `posted` punya cermin di rahaza_journal_lines
    # dengan Σdebit == Σcredit == total kepala (GL/TB membaca cermin ini tanpa filter status).
    jls_full = await db.rahaza_journal_lines.find(
        {}, {"_id": 0, "je_id": 1, "debit": 1, "credit": 1}).to_list(200000)
    mirror: dict = {}
    for jl in jls_full:
        slot = mirror.setdefault(jl.get("je_id"), [0.0, 0.0])
        slot[0] += _num(jl.get("debit"))
        slot[1] += _num(jl.get("credit"))
    v_jl2 = []
    posted_n = 0
    for je in jes:
        if je.get("status") != "posted":
            if je.get("id") in mirror and je.get("status") == "voided":
                v_jl2.append(f"{je.get('je_number')}(voided tapi masih punya cermin)")
            continue
        posted_n += 1
        m = mirror.get(je.get("id"))
        tag = je.get("je_number", je.get("id"))
        if not m:
            v_jl2.append(f"{tag}(posted tanpa cermin)")
        elif abs(m[0] - _num(je.get("total_debit"))) > EPS or abs(m[1] - _num(je.get("total_credit"))) > EPS:
            v_jl2.append(f"{tag}(cermin D{m[0]:.0f}/C{m[1]:.0f} != kepala "
                         f"D{_num(je.get('total_debit')):.0f}/C{_num(je.get('total_credit')):.0f})")
    _report("INV-JL-2 JE posted ↔ cermin journal_lines (ada & seimbang; voided tanpa cermin)", v_jl2, posted_n)

...
async def r4_r8_fungsional(db, ctx):
    ...
    mid = mat["id"]
    ctx["material_id"] = mid
    # Lokasi tujuan retur dicari lewat PERAN (fg/karantina), bukan kode ZNA-* bawaan —
    # sejak SEED_DEFAULT_LOCATIONS=false kodenya milik owner (mis. GD-L1-RAK / GD-L1-QC).
    loc_fg = await rb.resolve_location(db, "Baik")
    loc_qc = await rb.resolve_location(db, "Rusak")
    ctx["loc_fg_code"], ctx["loc_qc_code"] = loc_fg["code"], loc_qc["code"]
    ...
    if (res.get("created") and wh.get("restocked")
            and wh.get("restock_location_code") == loc_fg["code"]
    ...
    if (wh4.get("restocked") and wh4.get("restock_location_code") == loc_qc["code"]
...
async def cleanup(db, ctx):
    ...
    async for r in db.rahaza_material_stock.find(
        ...
        if r.get("location_code") == ctx.get("loc_qc_code", rb.LOC_QUARANTINE):

...
WRITE_PATHS = {
    ...
    "rahaza_customers.code": "backend/routes/sales_direct.py",
    # 2026-09-23 — Shipment Vendor (Kirim Material ke CMT) ditegakkan
    "vendor_shipments.shipment_number": "backend/routes/vendor_shipment.py",
}
...
FORM_PATHS = {
    "vendor_shipments.shipment_number":
        "frontend/src/components/erp/engine/VendorShipmentModule.jsx",
    "rahaza_customers.code":
        "frontend/src/components/erp/sales/SalesCustomersModule.jsx",
...
def part_static():
    ...
    def _form_terjangkau(rel: str) -> bool:
        nama = Path(rel).stem
        # `lazy(...)` maupun `lazyRetry(...)` (SESI #10: semua lazy() → lazyRetry())
        alias = re.findall(rf"const\s+(\w+)\s*=\s*lazy(?:Retry)?\(\(\)\s*=>\s*import\('[^']*{nama}'\)", reg_src)

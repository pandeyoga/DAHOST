...
Endpoints:
  GET  /api/capacity/config             — parameter kapasitas (T-17)
  PUT  /api/capacity/config             — ubah parameter (PRODUCTION_ROLES)
...
from core.authz import only
from core.roles import PRODUCTION_ROLES
...
@router.get("/config")
async def get_capacity_config(request: Request):
    """T-17 (FASE 3): parameter kapasitas bisa dilihat & diubah (dulu `capacity_config` tak punya penulis)."""
    await require_auth(request)
    return {"ok": True, "config": await _get_config(get_db()), "defaults": DEFAULT_CONFIG}


class CapacityConfigIn(BaseModel):
    daily_capacity_pcs: Optional[int] = Field(default=None, ge=1)
    overload_threshold: Optional[float] = Field(default=None, gt=0, le=5)
    critical_threshold: Optional[float] = Field(default=None, gt=0, le=5)
    lead_time_buffer_days: Optional[int] = Field(default=None, ge=0, le=60)
    working_hours_per_day: Optional[int] = Field(default=None, ge=1, le=24)
    shifts_per_day: Optional[int] = Field(default=None, ge=1, le=4)


@router.put("/config", dependencies=only(*PRODUCTION_ROLES))
async def update_capacity_config(body: CapacityConfigIn, request: Request):
    user = await require_auth(request)
    db = get_db()
    patch = {k: v for k, v in body.model_dump().items() if v is not None}
    if not patch:
        raise HTTPException(400, "Tidak ada parameter yang diubah")
    cur = await _get_config(db)
    if patch.get("critical_threshold", cur["critical_threshold"]) <= patch.get("overload_threshold", cur["overload_threshold"]):
        raise HTTPException(400, "critical_threshold harus lebih besar dari overload_threshold")
    patch.update({"updated_at": _now().isoformat(), "updated_by": user.get("id"), "updated_by_name": user.get("name", "")})
    await db.capacity_config.update_one({"_singleton": True}, {"$set": patch}, upsert=True)
    return {"ok": True, "config": await _get_config(db)}

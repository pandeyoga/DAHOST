...
def _txn_doc(session_id: str, txn_date: str, description: str, reference: str, amount: float, direction: str) -> dict:
    return {
        ...
        # FIN-18: sidik jari baris (tanggal+jumlah+arah+referensi/deskripsi) untuk cek kembar saat impor
        "fingerprint": f"{txn_date[:10]}|{round(abs(float(amount or 0)), 2)}|{direction}|{(reference or description or '').strip().lower()[:80]}",
    }


async def _drop_duplicate_txns(db, session_id: str, docs: list) -> tuple[list, int]:
    """FIN-18: buang baris yang sidik jarinya sudah ada di sesi (atau kembar di dalam berkas yang sama)."""
    if not docs:
        return docs, 0
    fps = [d["fingerprint"] for d in docs]
    existing = {t["fingerprint"] async for t in db.bank_recon_txns.find(
        {"session_id": session_id, "fingerprint": {"$in": fps}}, {"_id": 0, "fingerprint": 1})}
    out, seen, dup = [], set(), 0
    for d in docs:
        if d["fingerprint"] in existing or d["fingerprint"] in seen:
            dup += 1
            continue
...
async def _gl_balance_until(db, code: str, end: str) -> float:
    # FASE 4 (T-11): satu sumber dengan gl_balances_by_code & neraca saldo = cermin
    # rahaza_journal_lines (hanya JE posted yang punya cermin; void menghapusnya).
    rows = await db.rahaza_journal_lines.aggregate([
        {"$match": {"account_code": code, "date": {"$lte": end}}},
        {"$group": {"_id": None, "debit": {"$sum": {"$ifNull": ["$debit", 0]}},
                    "credit": {"$sum": {"$ifNull": ["$credit", 0]}}}},
    ]).to_list(1)
    if not rows:
        return 0.0
    return round(float(rows[0].get("debit") or 0) - float(rows[0].get("credit") or 0), 2)
...
async def delete_session(session_id: str, request: Request):
    ...
    await db.marketing_platform_withdrawals.update_many({"bank_session_id": session_id}, {"$set": {
...
async def add_transaction(session_id: str, request: Request):
    ...
    kept, dup = await _drop_duplicate_txns(db, session_id, [doc])
    if dup and not body.get("force"):
        raise HTTPException(409, "Mutasi kembar (tanggal, jumlah, referensi sama) sudah ada di sesi ini. Kirim force=true bila memang berbeda.")  # FIN-18
...
async def import_bulk_transactions(session_id: str, request: Request):
    ...
    docs, _dup_bulk = await _drop_duplicate_txns(db, session_id, docs)  # FIN-18
    if docs:
...
async def _release_txn(db, session_id: str, txn_id: str, user: dict | None = None):
    txn = await db.bank_recon_txns.find_one({"id": txn_id, "session_id": session_id}, {"_id": 0})
    # FIN-18: baris penyesuaian dilepas → jurnal penyesuaian & mutasi kasnya di-void
    if txn and txn.get("match_type") == "adjustment":
        from routes.rahaza_posting import _void_je_by_source
        async for adj in db.rahaza_bank_recon_adjustments.find({"txn_id": txn_id, "status": {"$ne": "voided"}}, {"_id": 0, "id": 1}):
            await _void_je_by_source(db, "bank_recon_adjustment", f"bank_adj:{adj['id']}", user or {"id": "system", "name": "release"}, "Lepas penyesuaian rekonsiliasi")
            await db.rahaza_bank_recon_adjustments.update_one({"id": adj["id"]}, {"$set": {"status": "voided", "voided_at": _now()}})
            await db.rahaza_cash_movements.delete_many({"ref_id": adj["id"], "category": "bank_adjustment"})
    if txn and txn.get("match_type") in ("settlement", "withdrawal") and txn.get("match_id"):
        await db.marketing_platform_withdrawals.update_one(
...
async def adjust_from_txn(session_id: str, txn_id: str, request: Request):
    ...
    if await db.rahaza_bank_recon_adjustments.find_one({"txn_id": txn_id, "status": {"$ne": "voided"}}, {"_id": 0, "id": 1}):
        raise HTTPException(409, "Mutasi ini sudah punya penyesuaian. Lepas dahulu (jurnalnya di-void) sebelum membuat yang baru.")  # FIN-18
    body = await request.json()
...
# ═══════════════════════════════════════════════════════════════════
# TAUTAN MUTASI BANK ↔ PENARIKAN SALDO PLATFORM (tahap 2 pencairan marketplace)
# Yang masuk rekening adalah PENARIKAN saldo dari Shopee/TikTok — bukan laporan
# dana dilepas (itu baru saldo di platform). Kunci respons dipertahankan
# (`settlement_id`/`net_payout`/`settlement_date`) supaya layar rekonsiliasi tetap jalan.
# ═══════════════════════════════════════════════════════════════════

def _wd_view(r: dict) -> dict:
    return {**r, "settlement_id": r.get("reference"), "net_payout": r.get("amount"),
            "settlement_date": r.get("withdrawal_date")}
...
async def settlement_candidates(session_id: str, txn_id: str, request: Request):
    ...
    t_dir = txn.get("direction") or ("in" if txn.get("type", "debit") == "debit" else "out")
    if t_dir != "in":
        raise HTTPException(400, "Penarikan saldo marketplace adalah uang MASUK — baris mutasi ini adalah uang keluar.")
    amount = float(txn.get("amount") or 0)
    rows = [_wd_view(r) for r in await db.marketing_platform_withdrawals.find(
        {"$or": [{"bank_txn_id": None}, {"bank_txn_id": {"$exists": False}}, {"bank_txn_id": txn_id}]},
        {"_id": 0, "id": 1, "reference": 1, "withdrawal_date": 1, "amount": 1, "platform": 1,
         "account_id": 1, "je_number": 1, "je_status": 1, "bank_txn_id": 1}).to_list(2000)]
...
async def link_settlement(session_id: str, request: Request):
    ...
    if t_dir != "in":
        raise HTTPException(400, "Penarikan saldo marketplace adalah uang MASUK — baris mutasi ini adalah uang keluar.")
    raw = await db.marketing_platform_withdrawals.find_one({"id": sdoc_id}, {"_id": 0})
    if not raw:
        raise HTTPException(404, "Penarikan saldo tidak ditemukan.")
    st = _wd_view(raw)
    if st.get("bank_txn_id") and st.get("bank_txn_id") != txn_id:
        raise HTTPException(409, f"Penarikan {st.get('settlement_id')} sudah tertaut ke mutasi lain (tanggal {st.get('bank_txn_date')}).")
    ...
    if abs(diff) > SETTLEMENT_AMOUNT_TOLERANCE:
        ...
        raise HTTPException(400, f"Nominal berbeda: mutasi bank {rp(txn.get('amount'))} vs penarikan {st.get('settlement_id')} "
                                 f"{rp(st.get('net_payout'))} (selisih {rp(diff)}). Koreksi nominal penarikan di Pencairan "
                                 f"Marketplace → Penarikan Saldo agar sama dengan mutasi bank, lalu tautkan lagi.")
    now = _now()
    ref = f"Penarikan {st.get('settlement_id')} · {st.get('platform') or ''}".strip(" ·")
    await db.bank_recon_txns.update_one({"id": txn_id}, {"$set": {
        "is_matched": True, "match_id": sdoc_id, "match_ref": ref, "match_type": "withdrawal", "matched_at": now}})
    await db.bank_recon_matches.insert_one({"id": _uid(), "session_id": session_id, "txn_id": txn_id, "target_type": "withdrawal",
    ...
    await db.marketing_platform_withdrawals.update_one({"id": sdoc_id}, {"$set": {
    ...
    return {"ok": True, "txn_id": txn_id, "settlement_doc_id": sdoc_id, "match_ref": ref,
            "message": f"Mutasi {txn.get('txn_date')} ditautkan ke penarikan {st.get('settlement_id')}."}
...
def _parse_idr(val: str) -> float:
    ...
    if re.search(r",\d{1,2}$", val):            # 1.500.000,50 (format ID)
        val = val.replace(".", "").replace(",", ".")
    elif re.search(r"\.\d{1,2}$", val) and val.count(".") == 1:  # FIN-10: 1,500,000.50 / 1500000.50 (format EN) — titik = desimal
        val = val.replace(",", "")
    else:
        val = val.replace(".", "").replace(",", "")
    try:
...
async def import_csv_file(session_id: str, request: Request, file: UploadFile = File(...)):
    ...
    docs, _dup_csv = await _drop_duplicate_txns(db, session_id, docs)  # FIN-18: unggah berkas sama 2× tidak menggandakan
    skipped += _dup_csv
    if docs:
        await db.bank_recon_txns.insert_many(docs)
        for d in docs:

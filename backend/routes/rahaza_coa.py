...
import re
from core.roles import FINANCE_ROLES  # FIN-02
...
async def _require_fin(request: Request):
    ...
    if role in FINANCE_ROLES:  # FIN-02
        return user
...
def _cf_group(value, acc: dict) -> str:
    """Kelompok arus kas OPR/INV/PND — dari input (divalidasi) atau bawaan kode/tipe akun."""
    from core.fin_statements import CF_GROUPS, default_cash_flow_group
    v = (value or "").strip().upper() if isinstance(value, str) else ""
    if v and v not in CF_GROUPS:
        raise HTTPException(400, f"cash_flow_group harus salah satu dari {list(CF_GROUPS)}")
    return v or default_cash_flow_group(acc)
...
async def create_account(request: Request):
    ...
    doc = {
        ...
        "cash_flow_group": _cf_group(body.get("cash_flow_group"), {"code": code, "type": acc_type, "flags": body.get("flags") or {}}),
        "active": True,
        "created_at": _now(),
        "updated_at": _now(),
        "created_by": user["id"],
...
async def update_account(aid: str, request: Request):
    ...
    if "cash_flow_group" in body:
        update["cash_flow_group"] = _cf_group(body["cash_flow_group"], acc)
...
# 2026-09-12 (owner): SATU skema 4 digit. Peta lengkap 3 digit → 4 digit ada di data/coa_unified.py
from data.coa_unified import DA_UNIFIED_ACCOUNTS, LEGACY_TO_UNIFIED, UNIFIED_GROUP_CODES, unified_code  # noqa: E402
LEGACY_CODE_MAP = dict(LEGACY_TO_UNIFIED)
...
async def migrate_coa_canonical(db) -> dict:
    ...
    # 2026-09-12: profil posting, rekening kas/bank, kategori expense → kode 4 digit tunggal
    report["posting_profile_remapped"] = 0
    async for pp in db.rahaza_posting_profiles.find({}, {"_id": 0, "id": 1, "mapping": 1}):
        mp = dict(pp.get("mapping") or {})
        new_mp = {k: (unified_code(v) if isinstance(v, str) else v) for k, v in mp.items()}
        if new_mp != mp:
            await db.rahaza_posting_profiles.update_one({"id": pp["id"]}, {"$set": {"mapping": new_mp, "updated_at": _now()}})
            report["posting_profile_remapped"] += 1
    report["cash_account_remapped"] = 0
    async for ca in db.rahaza_cash_accounts.find({"gl_account_code": {"$regex": r"^\d-\d{3}($|-)"}}, {"_id": 0, "id": 1, "gl_account_code": 1}):
        nc = unified_code(ca["gl_account_code"])
        if nc != ca["gl_account_code"]:
            await db.rahaza_cash_accounts.update_one({"id": ca["id"]}, {"$set": {"gl_account_code": nc}})
            report["cash_account_remapped"] += 1
    report["expense_category_remapped"] = 0
    if "rahaza_expense_categories" in await db.list_collection_names():
        async for ec in db.rahaza_expense_categories.find({}, {"_id": 0}):
            upd = {k: unified_code(v) for k, v in ec.items()
                   if isinstance(v, str) and k != "id" and __import__("re").match(r"^\d-\d{3}($|-)", v)}
            if upd:
                await db.rahaza_expense_categories.update_one({"id": ec["id"]}, {"$set": upd})
                report["expense_category_remapped"] += 1
    used_codes = {c for c in used_codes if c not in LEGACY_CODE_MAP or c in
                  set(await db.rahaza_journal_lines.distinct("account_code"))}
    ...
    # 2026-09-12 (owner: "jangan ada dua"): akun 3 digit yang sudah nonaktif, tak pernah dipakai
    # jurnal/rekening kas, dan tak punya sub-ledger → DIHAPUS supaya pemilih akun hanya 1 skema.
    purge = []
    async for a in db.rahaza_coa_accounts.find({"active": False, "flags.legacy_of": {"$exists": True}}, {"_id": 0, "code": 1}):
        c = a["code"]
        if c in used_codes:
            continue
        if await db.rahaza_coa_accounts.count_documents({"parent_code": c}) or \
           await db.rahaza_coa_accounts.count_documents({"code": {"$regex": f"^{c}-"}}):
            continue
        purge.append(c)
    if purge:
        await db.rahaza_coa_accounts.delete_many({"code": {"$in": purge}})
    report["legacy_purged"] = len(purge)
    return report
...
async def seed_da_coa(request: Request, force: bool = False):
    """Semai bagan akun tunggal 4 digit (SEED_TEMPLATE + akun khas DA).
    ...
    rep = await seed_coa_accounts(db)
    await migrate_coa_canonical(db)
    return {"ok": True, **rep}


async def seed_coa_accounts(db) -> dict:
    """Seed COA idempoten (by code) — dipakai auto-seed startup server.py dan endpoint admin.
    2026-09-12 (owner): SATU skema 4 digit — SEED_TEMPLATE (kanonik) + DA_UNIFIED_ACCOUNTS
    (akun khas DA: bank per entitas, dompet digital, penjualan per toko, HPP maklon, dst.).
    Skema 3 digit TIDAK disemai lagi; yang sudah ada dimigrasi oleh migrate_coa_canonical."""
    ...
    template_codes = [c for c, *_ in SEED_TEMPLATE if c not in DUP_TEMPLATE_CODES]
    for code, name, acc_type, is_group, flags in SEED_TEMPLATE:
        if code in DUP_TEMPLATE_CODES or code in existing_codes:
            skipped += 1
            continue
        ...
        inserted += 1
    for code, name, acc_type, is_group, parent_code, nb_override, cf_group in DA_UNIFIED_ACCOUNTS:
        ...
        await db.rahaza_coa_accounts.insert_one({
            ...
            "cash_flow_group": cf_group,
            "flags": {"is_contra": True} if nb_override else {}, "active": True,
            "created_at": _now(), "updated_at": _now(),
            "created_by": "system", "created_by_name": "auto-seed",
        })
        existing_codes.add(code)
        inserted += 1
    type_fixed = 0
    for gcode in UNIFIED_GROUP_CODES:
        r = await db.rahaza_coa_accounts.update_one({"code": gcode, "is_group": {"$ne": True}},
                                                    {"$set": {"is_group": True, "updated_at": _now()}})
        type_fixed += r.modified_count
    return {"inserted": inserted, "skipped": skipped, "type_fixed": type_fixed,
            "total_template": len(SEED_TEMPLATE) + len(DA_UNIFIED_ACCOUNTS)}

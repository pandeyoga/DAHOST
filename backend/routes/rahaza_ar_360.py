...
# FIN-09: satu pembaca kanonik — status terbuka = rahaza_ar_canonical.OPEN_STATUSES (draft & written_off BUKAN piutang)
from routes.rahaza_ar_canonical import OPEN_STATUSES as _CANON_OPEN
OPEN_AR_STATUSES = tuple(_CANON_OPEN) + ('partial', 'pending')
TERMINAL_AR_STATUSES = ('paid', 'cancelled', 'void', 'draft', 'written_off')
...
async def _fetch_open_invoices(db) -> List[Dict[str, Any]]:
    ...
    return await db.rahaza_ar_invoices.find(
        {'status': {'$in': list(OPEN_AR_STATUSES)}},  # FIN-09
        {'_id': 0}
...
def _classify_invoice(inv: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if balance <= 0 and inv.get('balance') is None:
        # FIN-09: cadangan hanya bila kolom balance memang tidak ada (jangan hidupkan kembali written_off/balance 0)

...
async def list_styles(
    ...
    # Jumlah varian nyata dari dewi_rnd_variants (kolom "Varian" dulu membaca style.variants yang tak pernah diisi)
    counts = {r['_id']: r['n'] async for r in db.dewi_rnd_variants.aggregate([
        {'$match': {'style_id': {'$in': [s['id'] for s in styles]}, 'status': {'$ne': 'deleted'}}},
        {'$group': {'_id': '$style_id', 'n': {'$sum': 1}}}])}
    out = []
    for s in styles:
        d = serialize(s)
        d['variants_count'] = counts.get(s['id'], len(s.get('variants') or []))
        out.append(d)
    return out
...
async def delete_style(style_id: str, user: dict = Depends(require_auth)):
    ...
    assert_can_act(user, 'rnd.manage', portal='rnd', what='menghapus data R&D')  # T-01 2.3
    db = get_db()
...
# ═══════════════════════════════════════════════════════════════════════════
# PAPAN KELENGKAPAN DATA (owner 2026-09-12) — per model: BOM semua varian · aksesoris di BOM · tech pack · foto · HPP · berat
# ═══════════════════════════════════════════════════════════════════════════
COMPLETENESS_CHECKS = [
    ("bom", "BOM semua varian"), ("accessories", "Aksesoris di BOM"), ("techpack", "Tech Pack"),
    ("photo", "Foto"), ("hpp", "HPP tervalidasi"), ("weight", "Berat"), ("sop", "SOP"),
]


@router.get('/completeness')
async def data_completeness(user: dict = Depends(require_auth)):
    db = get_db()
    models = await db.rahaza_models.find({'active': {'$ne': False}}, {'_id': 0, 'id': 1, 'code': 1, 'name': 1, 'hpp': 1, 'weight_gram': 1,
                                                                     'image_paths': 1, 'sop_steps': 1, 'rnd_style_id': 1, 'category_name': 1, 'hpp_validation': 1}).to_list(5000)
    variants: dict = {}
    all_variant_models = set(await db.rahaza_model_variants.distinct('model_id'))
    async for v in db.rahaza_model_variants.find({'active': True}, {'_id': 0, 'model_id': 1, 'size_id': 1, 'color_code': 1}):
        variants.setdefault(v['model_id'], set()).add((v.get('size_id'), (v.get('color_code') or '').upper()))
    ...
    discontinued_n = 0
    for m in models:
        vs, bk = variants.get(m['id'], set()), bom_keys.get(m['id'], set())
        ...
        flags = {
            ...
            'weight': float(m.get('weight_gram') or 0) > 0,
            'sop': bool(m.get('sop_steps')),
        }
        missing = [] if discontinued else [k for k, ok in flags.items() if not ok]
        discontinued_n += discontinued
        for k in missing:
            summary[k] += 1
        rows.append({'model_id': m['id'], 'code': m['code'], 'name': m['name'], 'category': m.get('category_name'), 'style_id': m.get('rnd_style_id'),
                     'variants': len(vs), 'boms': len(bk), 'variants_without_bom': len(vs - bk), 'flags': flags, 'missing': missing,
                     'discontinued': discontinued,
                     'hpp_validation': (m.get('hpp_validation') or {}).get('status'), 'hpp_reasons': (m.get('hpp_validation') or {}).get('reasons') or [],
                     'score': None if discontinued else round(100 * (len(flags) - len(missing)) / len(flags))})
    rows.sort(key=lambda r: (r['score'] if r['score'] is not None else 101, r['code']))
    complete = sum(1 for r in rows if not r['missing'] and not r['discontinued'])
    return {'checks': [{'key': k, 'label': l} for k, l in COMPLETENESS_CHECKS], 'total_models': len(rows), 'complete_models': complete,
            'discontinued_models': discontinued_n,
            'missing_counts': summary, 'hpp_unvalidated': sum(1 for r in rows if r['hpp_validation'] == 'belum_tervalidasi'), 'rows': rows}

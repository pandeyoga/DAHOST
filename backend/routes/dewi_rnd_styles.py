...
async def data_completeness(user: dict = Depends(require_auth)):
    ...
    bom_keys: dict = {}
    bom_acc: set = set()
    async for b in db.rahaza_boms.find({'active': {'$ne': False}, 'is_active': True}, {'_id': 0, 'model_id': 1, 'size_id': 1, 'color_code': 1, 'materials': 1}):
        bom_keys.setdefault(b['model_id'], set()).add((b.get('size_id'), (b.get('color_code') or '').upper()))
        if any(not ln.get('is_cut_panel') and (ln.get('material_type') or '').lower() != 'fabric' for ln in b.get('materials') or []):
            bom_acc.add(b['model_id'])
    ...
    for m in models:
        ...
        rows.append({'model_id': m['id'], 'code': m['code'], 'name': m['name'], 'category': m.get('category_name'), 'style_id': m.get('rnd_style_id'),
                     ...
                     'score': round(100 * (len(flags) - len(missing)) / len(flags))})

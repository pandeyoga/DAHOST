...
async def update_maklon_po(po_id: str, payload: MaklonPOUpdateIn, user: dict = Depends(require_auth)):
    ...
    if not po:
        raise HTTPException(404, 'PO tidak ditemukan')
    # T-06: mirror dari `production_pos` hanya boleh diubah lewat SSOT-nya —
    # perubahan di sini akan ditimpa ulang oleh production_maklon_bridge.
    if po.get('mirror_of') == 'production_pos':
        raise HTTPException(409, {
            'error': 'PO_IS_MIRROR',
            'message': 'PO ini cermin PO produksi — ubah di Portal Produksi (layar PO Produksi).',
            'production_po_id': po.get('production_po_id') or po_id,
            'target': 'maklon-pos-engine',
        })

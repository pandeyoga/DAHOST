...
async def seed_all_accounting_endpoint(request: Request):
    ...
    try:
        ...
        await seed_default_categories(user={"role": "superadmin", "id": "system", "name": "seed"})  # FIN-21: fungsi rute butuh user eksplisit
...
async def get_audit_findings(request: Request):
    """Papan Temuan Audit 2026-09-23 — status 30 temuan (owner/admin memantau mana yang sudah beres)."""
    await _require_super(request)
    from collections import Counter
    from data.audit_findings_2026_09_23 import FINDINGS
    by_status = Counter(f["status"] for f in FINDINGS)
    by_portal = Counter(f["portal"] for f in FINDINGS)
    return {"total": len(FINDINGS), "by_status": dict(by_status), "by_portal": dict(by_portal),
            "sumber": ["memory/temuan_2026-09-23/TEMUAN_PORTAL_FINANCE_1.md", "memory/temuan_2026-09-23/TEMUAN_PORTAL_PRODUKSI_MAKLON.md",
                       "memory/temuan_2026-09-23/TEMUAN_BUG_PORTAL_RND.md", "memory/PLAN_PERBAIKAN_TEMUAN_2026-09-23.md"],
            "items": FINDINGS}

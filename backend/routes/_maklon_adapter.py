...
def po_to_legacy_order(po: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        # MAK-01 (temuan 2026-09-23): angka per tahap yang ditulis PUT /stage-qty disimpan sebagai
        # `legacy_stage_qty` oleh penerjemah update, tetapi tidak pernah dibaca kembali → form kosong
        # & gerbang QC selalu 0. Gabungkan: nilai tersimpan + agregat items[*].
        'stage_qty': {
            **{k: v for k, v in (po.get('legacy_stage_qty') or {}).items() if isinstance(po.get('legacy_stage_qty'), dict)},

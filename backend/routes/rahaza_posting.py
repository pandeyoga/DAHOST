...
async def _create_posted_je(
    ...
    **DRAFT** dulu, bukan langsung `posted`. Alasannya bukan birokrasi: angka
    ...
    for i, ln in enumerate(lines_raw):
        ...
        if not (math.isfinite(d_amt) and math.isfinite(c_amt)):  # FIN-15
            return {"ok": False, "error": f"Baris #{i+1}: nilai tidak terhingga (inf/nan) ditolak."}
    ...
    # mirror lines for fast GL/TB — FASE 4 (T-09.2): cermin ditulis DULU, kepala TERAKHIR.
    # Kegagalan di tengah menyisakan baris yatim (terdeteksi INV-JL-1) — bukan kepala
    # `posted` tanpa cermin yang membuat buku besar diam-diam kurang.
    rows = [{
        "id": _uid(),
        "je_id": je_id,
        "je_number": je_number,
    ...
    try:
        ...
        # T-10: indeks unique parsial (source_module, source_ref, status≠voided) —
        # dua permintaan paralel dari sumber yang sama → kembalikan JE yang menang;
        # cermin milik JE yang kalah dibuang supaya tidak menggandakan saldo.
        if status == "posted" and rows:
            await db.rahaza_journal_lines.delete_many({"je_id": je_id})
        existing = await _find_existing_je(db, source_module, source_ref)
        if existing:
...
async def post_expense(db, expense: dict, user: dict) -> dict:
    ...
    if cash_acc_id:
        ...
        if cash_acc and cash_acc.get("gl_account_code"):
            ...
            # FIN-20f: rekening dipilih tapi "Tidak link" ke GL → jangan diam-diam mengkredit bank default
            result = {"ok": False, "error": f"Rekening '{cash_acc.get('name', cash_acc_id)}' belum ditautkan ke akun GL. Tautkan di Rekening Kas/Bank dahulu."}
...
async def post_asset_disposal(db, asset: dict, user: dict) -> dict:
    ...
    # FIN-06(c): kredit akun aset MILIK aset itu (account_id_asset) bila ada, bukan akun bawaan profil
    if asset.get("account_id_asset"):
        _own = await db.rahaza_coa_accounts.find_one({"id": asset["account_id_asset"], "active": True}, {"_id": 0, "code": 1})
        if _own and _own.get("code"):
            credit_asset_code = _own["code"]

...
async def download_file(path: str, request: Request, auth: str = Query(None)):
    ...
    tangan diverifikasi lewat `auth.verify_download_token` — satu jalur dengan
    ...
    if not raw or not verify_download_token(raw):
...
async def delete_attachment(att_id: str, request: Request):
    ...
    # T-01 2.3: hanya pengunggah, atau admin/owner/manager, yang boleh menghapus lampiran.
    role = (user.get('role') or '').lower()
    if doc.get('uploaded_by_id') not in (None, '', user.get('id')) and role not in (
            'superadmin', 'admin', 'owner', 'manager'):
        raise HTTPException(403, 'Akses ditolak: lampiran ini diunggah orang lain.')
    # Soft delete

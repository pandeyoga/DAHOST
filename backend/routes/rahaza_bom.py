...
def _clean_materials(raw):
    ...
    for m in raw or []:
        ...
        # 2026-09-12: baris POTONGAN dipertahankan penandanya saat BOM disunting di layar
        if m.get("is_cut_panel") or item["code"].startswith("CUT-"):
            item["is_cut_panel"] = True
            item["source_material_code"] = (m.get("source_material_code") or "").strip().upper()
            if m.get("migrated_from_fabric"):
                item["migrated_from_fabric"] = m.get("migrated_from_fabric")
        cleaned.append(item)
    return cleaned
    ...
    if isinstance(mats, list) and mats:
        ...
        for m in mats:
            ...
            norm.append({
                ...
                # 2026-09-12: penanda baris POTONGAN (bahan utama BOM) + kain asalnya
                "is_cut_panel": bool(m.get("is_cut_panel")),
                "source_material_code": m.get("source_material_code") or "",
                "migrated_from_fabric": m.get("migrated_from_fabric"),
            })
        return norm
    out = []
...
def _is_kglike(m) -> bool:
    # 2026-09-12: baris POTONGAN (pcs) bukan kain kiloan — jangan dijumlahkan sebagai kg.
    if m.get("is_cut_panel") or str(m.get("code") or "").upper().startswith("CUT-"):
        return False
...
async def _enrich_bom(db, bom):
    ...
    bom["accessory_count"] = len([m for m in mats if not _is_kglike(m) and not m.get("is_cut_panel")])
    # 2026-09-12 (keputusan owner): bahan utama = POTONGAN (1 pcs), bukan kg kain.
    _panel = next((m for m in mats if m.get("is_cut_panel")), None)
    bom["panel_code"] = (_panel or {}).get("code") or ""
    bom["panel_material_id"] = (_panel or {}).get("material_id") or ""
    bom["panel_source_fabric"] = (_panel or {}).get("source_material_code") or ""
    return bom
...
async def get_model_bom(model_id: str, request: Request):
    ...
    # 2026-09-12 (owner): BOM per model × UKURAN × WARNA — semua warna tiap ukuran
    # ditampilkan (dulu satu warna per ukuran tertimpa; 472 resep terlihat 104).
    by_size: dict[str, list] = {}
    for b in boms:
        by_size.setdefault(b["size_id"], []).append(b)
    matrix = []
    ...
    def _row(s, b):
        ...
        return {
            ...
            "color": (b or {}).get("color") or "",
            "color_code": (b or {}).get("color_code") or "",
            ...
            **material_fields.mirror("bulk_line_count", len([m for m in mats if _is_kglike(m)])),
            "accessory_count": len([m for m in mats if not _is_kglike(m) and not m.get("is_cut_panel")]),
            # 2026-09-12: bahan utama = POTONGAN (1 pcs), bukan kg kain
            "panel_code": next((m.get("code") for m in mats if m.get("is_cut_panel")), ""),
            "panel_source_fabric": next((m.get("source_material_code") for m in mats if m.get("is_cut_panel")), ""),
            "notes":           b.get("notes", "") if b else "",
            "updated_at":      b.get("updated_at") if b else None,
        }

    for s in sizes:
        rows = sorted(by_size.get(s["id"]) or [], key=lambda b: (b.get("color_code") or b.get("color") or ""))
        if not rows:
            matrix.append(_row(s, None))
            continue
        for b in rows:
            matrix.append(_row(s, b))
    return {
        "model": {"id": model["id"], "code": model["code"], "name": model["name"]},
        "matrix": matrix,
        "bom_count": len(boms),
        "size_count": len({b["size_id"] for b in boms}),
        "color_count": len({(b.get("color_code") or b.get("color") or "") for b in boms if (b.get("color_code") or b.get("color"))}),
    }

...
async def _validate_lines(db, lines: list) -> tuple[float, float]:
    ...
    for i, ln in enumerate(lines):
        ...
        if not (math.isfinite(d) and math.isfinite(c)):  # FIN-15: inf/nan lolos float()
            raise HTTPException(400, f"Baris #{i+1}: debit/credit harus angka terhingga.")
        if d < 0 or c < 0:
...
async def post_journal(je_id: str, request: Request):
    ...
    await _check_period_open(db, je_date)
    # FIN-17: atomik — hanya satu permintaan yang mengubah draft→posted; baris disalin hanya bila menang
    res = await db.rahaza_journal_entries.update_one(
        {"id": je_id, "status": "draft"},
...
HUB_VOIDABLE_SOURCES = {"manual", "manual_journal", "general", "adjustment", "year_end_close",
                        "marketplace_settlement", "platform_withdrawal", "bank_recon", "bank_recon_adjustment"}

...
router = APIRouter(prefix="/api/rahaza/finance", tags=["rahaza-accruals"],
                   dependencies=[Depends(require_portal_dep("finance"))])  # FIN-04: pintu samping jurnal → portal keuangan
...
async def post_accrual(accrual_id: str, request: Request):
    ...
    try:
        ...
        raise  # FIN-20e: jangan dibungkus jadi 500
...
async def create_recurring_accruals(request: Request):
    ...
    # FIN-14: templat = akrual berulang ASLI (bukan anak yang punya recurring_template_id)
    query = {"is_recurring": True, "status": {"$ne": "draft"}, "recurring_template_id": {"$in": [None, ""]}}
    if template_ids:
    ...
    for template in templates:
        ...
        new_doc = {
            ...
            "status": "draft",
            "is_recurring": False,  # FIN-14: anak bukan templat
            "recurring_template_id": template.get("recurring_template_id") or template.get("id"),
            "posted_at": None,
            "posted_by": None,
            "je_id": None,
            "je_number": None,

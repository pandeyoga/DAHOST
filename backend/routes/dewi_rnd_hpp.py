...
from routes.shared import assert_can_act  # T-01 2.3
from core.roles import FINANCE_ROLES
...
async def delete_hpp(calc_id: str, user: dict = Depends(require_auth)):
    assert_can_act(user, 'rnd.manage', portal='rnd', legacy_roles=FINANCE_ROLES, what='menghapus data R&D')  # T-01 2.3
    db = get_db()
...
@router.post('/hpp-calculator/compute-from-bom', deprecated=True)
async def compute_hpp_from_bom(body: dict, user: dict = Depends(require_auth)):
    """RND-04 (temuan 2026-09-23): dipensiunkan — tanpa pemanggil FE; tumpang tindih dengan
    tombol "Tarik dari Techpack BOM" (/hpp-calculator/cost-lines/from-techpack)."""
    raise HTTPException(410, "Endpoint dipensiunkan (RND-04). Pakai /hpp-calculator/cost-lines/from-techpack.")
...
@router.post('/hpp-calculator/{calc_id}/propagate', deprecated=True)
async def propagate_hpp_endpoint(calc_id: str, user: dict = Depends(require_auth)):
    """RND-04: dipensiunkan — propagasi sudah otomatis saat HPP disimpan/disunting (_propagate_hpp)."""
    raise HTTPException(410, "Endpoint dipensiunkan (RND-04): propagasi HPP berjalan otomatis saat simpan/sunting.")
    db = get_db()
    doc = await db.dewi_rnd_hpp.find_one({'id': calc_id}, {'_id': 0})
    if not doc:
        raise HTTPException(404, 'HPP calculation tidak ditemukan')

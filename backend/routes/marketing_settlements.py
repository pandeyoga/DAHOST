...
═══════════════════════════════════════════════════════════════════════════════
DUA TAHAP (keputusan pemilik): SALDO PLATFORM → PENARIKAN KE BANK
═══════════════════════════════════════════════════════════════════════════════
Dokumen di berkas ini = TAHAP 1: dana yang DILEPAS platform ke saldo toko
(laporan Penghasilan Shopee / Settlement TikTok). Jurnalnya:
    Dr Piutang Toko `1-1303-xxx` (bersih)  ·  Dr potongan  ·  Cr Pendapatan toko (bruto)
Saldo `1-1303-xxx` per toko di neraca = SALDO YANG BISA DICAIRKAN.
TAHAP 2 (`routes/marketing_withdrawals.py`): penarikan saldo ke rekening bank —
    Dr Bank toko / Cr Piutang Toko — dan inilah yang dicocokkan ke mutasi bank.
Omzet input Marketing tidak pernah menyentuh keduanya (gate INV-F42).
"""
...
COA = {
    "receivable":  "1-1303",  # Piutang Platform Online Shop (kontrol) — saldo di platform
    ...
    "platform_fee": "4-1400",  # Potongan Platform (Fee Shopee/TikTok)
    "ads":         "6-1130",  # Biaya Pemasaran Lain-lain (default; per platform lihat ADS_BY_PLATFORM)
...
# Potongan iklan dari pencairan = iklan platform itu sendiri (6-1100 kini akun induk).
ADS_BY_PLATFORM = {"shopee": "6-1112", "tiktok": "6-1110", "tiktokshop": "6-1110", "tokopedia": "6-1113"}
...
class SettlementIn(BaseModel):
    ...
    settlement_date: str                 # YYYY-MM-DD — tanggal dana dilepas ke saldo platform
...
ACCOUNT_COA_ROLES = {
    "receivable": (("ar_account_code", "coa_receivable_code"), "Akun Saldo Platform / Piutang Toko"),
    "revenue": (("coa_revenue_code",), "Akun Pendapatan"),
}


async def _resolve_coa(db, account: dict) -> dict:
    """Peta akun EFEKTIF untuk satu laporan dana dilepas. Melempar 400 bila toko
    belum punya akun piutang toko/pendapatan sendiri — TIDAK diam-diam memakai kontrol."""
    eff = dict(COA)
    sources = {k: "global" for k in COA}
    ads = ADS_BY_PLATFORM.get((account.get("platform") or "").lower())
    if ads and await db.rahaza_coa_accounts.find_one({"code": ads, "active": True, "is_group": {"$ne": True}}, {"_id": 0}):
        eff["ads"], sources["ads"] = ads, "platform"
    missing = []
    for role, (fields, label) in ACCOUNT_COA_ROLES.items():
        field = next((f for f in fields if (account.get(f) or "").strip()), fields[0])
        code = (account.get(field) or "").strip()
        if not code:
            missing.append(f"{label} (`{'`/`'.join(fields)}`)")
            continue
    ...
    if missing:
        raise HTTPException(
            ...
            f"Marketing → Kelola Akun (bagian Tautan Finance). Jurnal TIDAK dibuat "
            f"dengan akun bawaan, karena saldo dari toko yang berbeda akan jatuh ke "
            f"akun yang sama dan saldo per toko jadi tidak bisa dipertanggungjawabkan.")
...
async def platform_balance(request: Request):
    """Saldo per toko yang masih di platform = Σ dana dilepas − Σ penarikan ke bank.
    `gl_balance` = saldo akun `1-1303-xxx` di buku besar (hanya jurnal yang sudah posting)."""
    user = await require_auth(request)
    db = get_db()
    vis = await _scope.visible_account_ids(db, user)
    rows = await _bal.platform_balances(db, vis)
    return {"ok": True, "data": rows,
            "total_balance": round(sum(r["balance"] for r in rows), 2),
            "total_released": round(sum(r["released_total"] for r in rows), 2),
            "total_withdrawn": round(sum(r["withdrawn_total"] for r in rows), 2)}
...
async def update_settlement(sid: str, body: SettlementIn, request: Request):
    ...
    upd = body.dict()
    # Dana dilepas yang sudah DITARIK sebagian tidak boleh dikecilkan di bawah penarikannya —
    # saldo platform akan negatif dan penarikan yang sudah tercatat kehilangan sumbernya.
    new_net = round(float(upd.get("net_payout") or 0), 2)
    old_net = round(float(cur.get("net_payout") or 0), 2)
    if new_net < old_net:
        avail = await _bal.available_balance(db, cur["account_id"])
        if avail - (old_net - new_net) < -0.01:
            raise HTTPException(
                400, f"Nominal tidak bisa dikecilkan sebesar itu: saldo platform toko ini tinggal "
                     f"{_rp(avail)} — sisanya sudah ditarik ke bank. Koreksi/hapus penarikannya dulu.")
    if cur.get("je_id"):
...
async def delete_settlement(sid: str, request: Request):
    ...
    avail = await _bal.available_balance(db, cur["account_id"])
    if avail - float(cur.get("net_payout") or 0) < -0.01:
        raise HTTPException(
            400, f"Tidak bisa dihapus: dana ini sudah ikut ditarik ke bank (saldo platform toko "
                 f"tinggal {_rp(avail)}). Hapus/koreksi penarikannya dulu.")
...
async def create_draft_journal(sid: str, request: Request):
    ...
    lines = [
        # Dana bersih yang DILEPAS platform ke saldo toko — akun piutang MILIK TOKO ini.
        # Belum uang di bank: baru jadi kas saat ditarik (routes/marketing_withdrawals.py).
        {"account_code": ACC["receivable"], "debit": g("net_payout"), "credit": 0,
         "description": f"Dana dilepas {doc['platform']} ke saldo toko ({doc['settlement_id']})"},
    ...
    res = await _create_posted_je(
        ...
        memo=f"Dana dilepas {doc['platform']} ke saldo toko — {doc.get('account_name') or ''} "
             f"({doc['settlement_id']})",
    ...
    return {"ok": True, "je_id": res["je_id"], "je_number": res["je_number"],
            "je_status": "draft", "coa_used": ACC, "coa_source": coa["sources"],
            "message": f"Jurnal DRAFT {res['je_number']} dibuat: Dr akun saldo platform "
                       f"{ACC['receivable']} / Cr akun pendapatan {ACC['revenue']} milik toko "
...
async def reconcile(
    ...
    sq: dict = {}
    vis = await _scope.visible_account_ids(db, user)
    # FIN-19: lingkup toko tidak boleh ditimpa — settlement_id/account_id di luar lingkup → 403
    if vis is not None:
        if focus and focus.get("account_id") not in vis:
            raise HTTPException(403, "Pencairan ini milik toko di luar lingkup akses Anda.")
        if account_id and account_id not in vis:
            raise HTTPException(403, "Toko di luar lingkup akses Anda.")
    ...
    if account_id:
        if vis is not None and account_id not in vis:  # FIN-19
            raise HTTPException(403, "Toko di luar lingkup akses Anda.")
...
async def post_settlement_journal(sid: str, request: Request):
    ...
    _res = await db.rahaza_journal_entries.update_one(
        {"id": je["id"], "status": "draft"},  # FIN-17: atomik
        {"$set": {"status": "posted", "posted_at": _now(),
                  "posted_by": (user or {}).get("id"), "updated_at": _now()}})
    if _res.modified_count == 0:
        raise HTTPException(409, "Jurnal sudah diposting oleh permintaan lain.")
    je["status"] = "posted"
...
async def get_settlement(sid: str, request: Request):
    ...
    account = await db.marketing_platform_accounts.find_one(
        ...
        {"_id": 0, "id": 1, "account_name": 1, "platform": 1,
         "coa_cash_code": 1, "coa_revenue_code": 1, "coa_receivable_code": 1, "ar_account_code": 1}) or {}
    recv = _bal.receivable_code(account)
    coa_ready = bool(recv and (account.get("coa_revenue_code") or "").strip())
    ...
    return {
        ...
        "coa": {
            ...
            "deduction_accounts": {k: v for k, v in COA.items()
                                   if k not in ("receivable", "revenue")},
            "note": ("Dana bersih masuk ke akun saldo platform (piutang toko) dan pendapatan "
                     "diambil dari akun toko; akun potongan (retur, diskon, fee platform, "
                     "iklan, lain-lain) memakai peta global karena tidak per toko. Uang baru "
                     "jadi kas saat saldo ditarik ke bank (Penarikan Saldo)."
                     if coa_ready else
                     "Toko ini belum punya akun saldo platform / pendapatan — jurnal "
                     "akan DITOLAK sampai diisi di Portal Marketing → Kelola Akun."),
        },
        "can": {

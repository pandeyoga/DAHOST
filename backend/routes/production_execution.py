...
async def delete_job(jid: str, request: Request):
    ...
    # T-07: satu pintu — void JE WIP→FG, bersihkan layer/snapshot/cermin, cek periode terkunci.
    from core.production_job_delete import delete_production_job
    summary = await delete_production_job(db, jid, user, reason=f"Job {doc.get('job_number')} dihapus")
...
async def get_progress(request: Request):
    ...
    # FASE 4 (T-21): layar daftar dibatasi (`?limit=`, bawaan 2000 terbaru) — bukan tanpa batas.
    from core.pagination import LEGACY_DEFAULT_CAP
    limit = min(int(sp.get('limit') or LEGACY_DEFAULT_CAP * 2), 10000)
    return serialize_doc(await db.production_progress.find(query, {'_id': 0}).sort('progress_date', -1).to_list(limit))
...
async def create_progress(request: Request):
    ...
    if body.get('job_item_id'):
        ...
        # 2026-09-24 — gate bisa DIMATIKAN owner (setelan `production_require_material_issued`,
        # bawaan OFF sementara sampai alur gudang klien rapi).
        from routes.dewi_system_config import get_config_value
        _gate_on = await get_config_value(db, 'production_require_material_issued', False)
        if isinstance(_gate_on, str):
            _gate_on = _gate_on.strip().lower() in ('1', 'true', 'yes', 'on')
        if job_doc.get('business_type') == 'internal' and _gate_on:
            ...
            if not issued_mi:
                raise HTTPException(400,
                    ...
                    'progress produksi internal ditolak (GDG-2). Matikan validasi ini di '
                    'Konfigurasi Sistem → Produksi bila ingin progress tanpa MI.')
...
async def distribusi_kerja(request: Request):
    ...
    # FASE 4 (T-21): hierarki PO dibatasi 2000 PO terbaru (`?limit=`), bukan tanpa batas.
    _limit = min(int(sp.get('limit') or 2000), 10000)
    pos = await db.production_pos.find(po_query, {'_id': 0}).sort('created_at', -1).to_list(_limit)

...
async def vendor_shipment_material_preview(request: Request):
    ...
    for code, e in need.items():
        ...
        problem = ('belum punya baris stok di gudang' if not loc
                   else ('stok kurang' if avail + 1e-9 < e['qty'] else ''))
        if not loc and neg_ok:
            fb = await negative_stock.fallback_location(db, mat)
            if fb:
                loc, avail = fb['id'], 0.0
                problem = f"belum ada baris stok → akan dibuat minus di {fb.get('code')}"
        short = (not loc) or (avail + 1e-9 < e['qty'])
        if short:
            has_short = True
        if short and neg_ok and loc:
            problem = problem or 'stok kurang'
            problem += f" (akan minus {round(e['qty'] - avail, 4):g} {mat.get('unit') or e['unit']})"
        rows.append({
            ...
            'shortage': short,
            'blocking': short and not (neg_ok and loc),
            'problem': problem,
        })
    ...
    return {
        ...
        # mode sementara stok boleh minus: kekurangan = peringatan, bukan pemblokir
        'allow_negative': neg_ok,
        'blocking': any(r.get('blocking') for r in rows),
        'bom_notes': notes,
...
async def create_vendor_shipment(request: Request):
    ...
    # 2026-09-23 — nomor lewat kebijakan penomoran (otomatis/manual), diterbitkan
    # SETELAH semua validasi supaya nomor urut tidak terbuang oleh permintaan gagal.
    from core.doc_number_policy import issue_number
    body['shipment_number'] = await issue_number(
        db, 'vendor_shipments.shipment_number',
        requested=(body.get('shipment_number') or '').strip())
    shipment_id = new_id()
    shipment = {
    ...
    if _is_material_out:
        ...
        async def _rollback_shipment():
            ...
            # nomor otomatis yang batal dipakai dikembalikan → tidak ada lubang nomor
            from core.doc_number_policy import release_number
            await release_number(db, 'vendor_shipments.shipment_number', shipment['shipment_number'])
    ...
    if material_issue:
        result['material_issue'] = {
            ...
            'negative_stock_lines': material_issue.get('negative_stock_lines') or [],
        }
...
async def delete_vendor_shipment(sid: str, request: Request):
    ...
    # T-07: satu pintu hapus job (void JE, layer, snapshot, cermin; 409 bila periode terkunci)
    from core.production_job_delete import delete_production_job
    for job in jobs:
        await delete_production_job(db, job['id'], user, reason=f"Shipment {doc.get('shipment_number')} dihapus")

...
async def get_my_annual_review(request: Request):
    ...
    # KPI saya = KPI yang tertanam di assignment (dewi_perf_kpis hanya katalog, tanpa employee_id)
    kpis = [
        {**k, "assignment_id": a.get("id"), "cycle_id": a.get("cycle_id")}
        for a in assignments if a.get("employee_id") == emp_id

...
from auth import (require_auth, verify_password, hash_password,
...
async def issue_download_token(request: Request):
    """FASE 4 (T-22): token unduh 5 menit (aud=download) untuk URL window.open/<a href>/<img>.
    Body opsional: {"resource": "<path atau nama sumber>"} — token hanya sah untuk sumber itu."""
    ...
    from auth import DOWNLOAD_TOKEN_TTL_SEC, create_download_token
    return {"token": create_download_token(user, resource), "expires_in": DOWNLOAD_TOKEN_TTL_SEC, "resource": resource}
...
async def change_password(request: Request):
    """Ganti sandi sendiri. Wajib untuk akun impor master (`must_change_password`)."""
    user = await require_auth(request)
    body = await request.json()
    ...
    if len(new_pw) < 8 or new_pw.isalpha() or new_pw.isdigit():
        raise HTTPException(400, 'Kata sandi baru minimal 8 karakter dan memuat huruf serta angka/simbol')
    if new_pw == old_pw:
        raise HTTPException(400, 'Kata sandi baru harus berbeda dari kata sandi lama')
    await db.users.update_one({'id': u['id']}, {
        '$set': {'password': hash_password(new_pw), 'must_change_password': False,
                 'password_changed_at': datetime.now(timezone.utc)}})
    await log_activity(u['id'], u['name'], 'Ganti Sandi', 'Auth', f"User {u['email']} mengganti kata sandi")

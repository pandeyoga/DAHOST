...
async def release_number(db, key: str, number: str) -> bool:
    """Kembalikan nomor OTOMATIS yang dokumennya batal dibuat (rollback).

    Hanya bila nomor itu masih yang TERAKHIR diterbitkan — kalau sudah ada nomor
    lain sesudahnya, dibiarkan (tidak boleh mengacak urutan). Mode manual tidak
    menyentuh counter, jadi tidak ada yang perlu dikembalikan.
    """
    m = re.search(r"(\d+)\s*$", number or "")
    if not m:
        return False
    coll, field = target_of(_entry(key))
    ckey = f"autonum:{coll}:{field}:{number[:m.start(1)]}"
    res = await db.counters.update_one({"_id": ckey, "seq": int(m.group(1))},
                                       {"$inc": {"seq": -1}})

...
# Peran non-storage yang SAH memegang baris stok (WIP produksi / karantina).
EXEMPT_ROLES = ("cutting", "sewing", "qc", "packing", "karantina")
# Kode legacy seed bawaan per peran — fallback bila lokasi tidak punya `storage_role`.
LEGACY_ROLE_CODES = {"bahan": "ZNA-KAIN", "aksesoris": "ZNA-AKSESORIS", "fg": "ZNA-FG",
                     "sample": "ZNA-SAMPLE", "karantina": "ZNA-KARANTINA",
                     "cutting": "ZNA-CUTTING", "sewing": "ZNA-SEWING", "qc": "ZNA-QC",
                     "packing": "ZNA-PACKING"}
# Nama peran yang ditulis owner di template 01_LOKASI (kolom `peran`) → peran internal.
ROLE_ALIASES = {"kain": "bahan", "material": "bahan", "barang_jadi": "fg", "aksesoris": "aksesoris",
                "acc": "aksesoris", "sampel": "sample", "quarantine": "karantina", "jahit": "sewing",
                "potong": "cutting", "packing": "packing", "qc": "qc"}
ALL_LOCATION_ROLES = tuple(dict.fromkeys(STORAGE_ROLES + EXEMPT_ROLES))
...
def role_of_location(loc: dict) -> str:
    """Peran lokasi legacy: field `storage_role` (impor master) atau kode seed bawaan."""
    role = normalize_role(loc.get("storage_role") or loc.get("role"))
    if role in ALL_LOCATION_ROLES:
        return role
    code = (loc.get("code") or "").upper()
    for r, c in LEGACY_ROLE_CODES.items():
        if c == code:
            return r
    return ""


async def legacy_location_for_role(db, role: str) -> dict | None:
    """Lokasi `rahaza_locations` untuk satu peran: `storage_role` dulu, lalu kode seed lama.
    Pemakai fungsional (FG, karantina, aksesoris, retur) WAJIB lewat sini — tidak boleh
    mengandalkan kode `ZNA-*` yang hanya ada bila seed bawaan dinyalakan."""
    role = normalize_role(role)
    proj = {"_id": 0, "id": 1, "code": 1, "name": 1, "storage_role": 1}
    doc = await db.rahaza_locations.find_one(
        {"storage_role": role, "active": {"$ne": False}}, proj)
    if doc:
        return doc
    code = LEGACY_ROLE_CODES.get(role)
    return await db.rahaza_locations.find_one({"code": code}, proj) if code else None
...
async def list_storage_locations(db) -> list:
    ...
    for l in legacy:
        if l["id"] in mapped_ids or l["id"] in seen:
            continue
        l_role = role_of_location(l)
        if l_role not in STORAGE_ROLES:
            continue
        out.append({
            "id": l["id"],
            "code": l.get("code") or "",
...
async def storage_location_index(db) -> dict:
    ...
    for loc in legacy:
        ...
        l_role = role_of_location(loc)
        if l_role in STORAGE_ROLES:
            _put(loc["id"], code, loc.get("name"), "rahaza_location", KIND_STORAGE)
        elif l_role in EXEMPT_ROLES:
    ...
    for role in STORAGE_ROLES:
        ...
        if zid:
            ...
            loc = next((l for l in legacy if role_of_location(l) == role
                        and l.get("active", True) is not False), None)
            if loc:

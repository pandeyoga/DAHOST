...
def bom_rows_from_file(data: bytes) -> list[tuple]:
    ...
    if "BOM_AKSESORIS" not in wb.sheetnames:
        return []
    return [tuple(r) + (None,) * (9 - len(r)) for r in wb["BOM_AKSESORIS"].iter_rows(values_only=True, min_row=2)]
...
def materials_needing_pack(parsed: dict) -> dict[str, str]:
    ...
    for i in parsed.get("bom_issues") or []:
        if i["kategori"] == "satuan_tak_valid" and "isi_per_satuan_beli" in i.get("detail", "") and i.get("code"):
            out.setdefault(i["code"], f"isi kemasan (pcs per {i.get('unit_raw') or 'kemasan'}) belum ada — dipakai BOM {i.get('model_code')} baris {i['row']}")
    return out

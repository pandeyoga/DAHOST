...
def _shippable(r: dict) -> int:
    # Barang yang sama tidak boleh dihitung dua kali: bila baris sudah punya penerimaan
    # dari CMT (`good_from_cmt`), hasil produksi internal (`produced_qty`) adalah barang
    # yang SAMA yang lalu diterima — pakai yang lebih besar, bukan dijumlahkan
    # (keluhan owner 2026-09-24: diproduksi 32 tetapi sisa bisa kirim 64).
    made = max(r['good_from_cmt'], r['internal_produced']) if r['good_from_cmt'] else r['internal_produced']
    return max(0, made + r['reworked_ok'] - r['dispatched'])

...
async def ensure_panel_for_variant(db, variant: dict, model: dict | None = None) -> tuple[dict, bool]:
    """Keputusan owner 2026-09-24: master POTONGAN lahir OTOMATIS saat varian produk
    internal (SKU) dibuat — Portal Cutting tinggal mencocokkan potongan itu pakai kain apa
    (`source_material_*` diisi saat cutting). Idempoten; harga tetap 0 (`menunggu_cutting`)."""
    if model is None:
        model = await db.rahaza_models.find_one({"id": variant.get("model_id")}, {"_id": 0, "sop_steps": 0}) or {
            "id": variant.get("model_id"), "code": variant.get("model_code"), "name": variant.get("model_name")}
    return await ensure_panel(
        db, model=model, color_code=variant.get("color_code") or "",
        color_name=variant.get("color_name") or variant.get("color_code") or "",
        size_code=variant.get("size_code") or "", size_id=variant.get("size_id"),
        created_from="variant_auto")


async def ensure_bom_for_variant(db, variant: dict, model: dict, panel: dict, user: dict | None = None) -> dict:
    """Keputusan owner 2026-09-24: setiap varian baru langsung punya BOM ber-baris potongan.

    * Model sudah punya BOM lain → salin ke varian ini (kain/potongan disesuaikan warna,
      lewat `master_fill.bom_copy_to_missing`), lalu pastikan baris potongan ada.
    * Model belum punya BOM sama sekali → buat BOM minimal: 1 pcs potongan (aksesoris
      diisi R&D/BOM editor kemudian).
    Idempoten; BOM yang sudah punya baris potongan tidak disentuh.
    """
    key = {"model_id": variant.get("model_id"), "size_id": variant.get("size_id"),
           "color_code": (variant.get("color_code") or "").upper(), "active": {"$ne": False}}
    bom = await db.rahaza_boms.find_one(key, {"_id": 0})
    if not bom:
        from core.master_fill import bom_copy_to_missing
        try:
            await bom_copy_to_missing(db, user, model_id=variant.get("model_id"))
        except Exception:  # noqa: BLE001 — salinan gagal ⇒ jatuh ke BOM minimal
            pass
        bom = await db.rahaza_boms.find_one(key, {"_id": 0})
    if not bom:
        bom = {"id": str(uuid.uuid4()), "model_id": variant.get("model_id"), "size_id": variant.get("size_id"),
               "color": variant.get("color_name") or variant.get("color_code") or "",
               "color_code": (variant.get("color_code") or "").upper(),
               "version": 1, "is_active": True, "active": True,
               "materials": [bom_line_for_panel(panel)],
               "notes": "BOM awal otomatis saat varian dibuat — 1 pcs potongan; aksesoris menyusul",
               "created_by": (user or {}).get("id", "system"), "created_at": _now(), "updated_at": _now()}
        await db.rahaza_boms.insert_one(dict(bom))
        return {"bom_id": bom["id"], "action": "created_minimal"}
    if any(is_panel_line(ln) for ln in bom.get("materials") or []):
        return {"bom_id": bom["id"], "action": "ok"}
    await db.rahaza_boms.update_one(
        {"id": bom["id"]},
        {"$set": {"materials": (bom.get("materials") or []) + [bom_line_for_panel(panel)], "updated_at": _now()},
         "$push": {"change_log": {"at": _now(), "by": (user or {}).get("name", "system"),
                                  "note": f"+1 pcs potongan {panel.get('code')} (otomatis)"}}})
    return {"bom_id": bom["id"], "action": "panel_line_added"}

...
async def inspect(db, panel: dict, *, onhand: float | None = None) -> dict:
    ...
    # 2026-09-12: potongan yang lahir dari BOM/impor (`created_from != 'cutting'`) memang
    # belum punya order cutting — itu bahan master produk, BUKAN yatim.
    owned_by_bom = (panel.get("created_from") or "cutting") != "cutting"
    if orders_ref == 0 and not maker and not owned_by_bom:
...
async def remove_if_unused(db, *, panel_id: str, order_id: str,
    ...
    if not panel or not panel.get("is_cut_panel"):
        return None
    # hanya master yang MEMANG dibuat order ini (bukti kepemilikan). Potongan yang
    # lahir dari BOM/impor (2026-09-12, `created_from != 'cutting'`) milik master
    # produk — tidak pernah ikut dibuang bersama order.
    if (panel.get("created_from") or "cutting") != "cutting":
        return None

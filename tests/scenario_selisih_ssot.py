...
def make_po(sku: str, qty: int, label: str, buyer="UJI Buyer Selisih"):
    ...
    st, po = call("POST", "/api/production-pos", {
        "business_type": "maklon", "vendor_id": VENDOR_ID,
...
def main():  # noqa: C901
    ...
    # FASE F (2026-08-15): subtotal per PO DIHAPUS atas keputusan pemilik (rekap kumulatif);
    # yang wajib: kolom "No. PO" ada dan tidak ada baris SUBTOTAL yang tumpang tindih.
    check("D1d PDF punya kolom No. PO tanpa baris SUBTOTAL (keputusan pemilik FASE F)",
          "NO. PO" in txt.upper() and txt.upper().count("SUBTOTAL") == 0,

...
- **Waktu:** 2026-09-12 11:56:11  ·  **Durasi:** 145s  ·  **Mode:** cepat
...
| UANG — batas nilai AR/AP/maklon (round6) | FAIL |
...
| KEAMANAN/UANG — Portal CMT Override: kewenangan, scoping, jejak (INV-CMTOV) | FAIL |
| UANG/PRODUK — Rekap Harian + Mingguan CMT: batas WIB, definisi terisi, SSOT export (INV-REKAP) | FAIL |
...
| UANG/ARAH — Marketing: siklus target·anggaran·omzet + kunci periode (INV-MKTCYCLE) | FAIL |
| DATA/UANG — Marketing: impor KPI Seller Center + assign toko + scorecard (INV-KPIIMPOR) | FAIL |
...
| KEAMANAN/DATA — Marketing: lingkup toko per pemakai + jejak perubahan (INV-F6RBAC) | FAIL |
| BISA DIPAKAI — Marketing: layar daftar (tabel · cari · unduh) + field tak disembunyikan (INV-F10) | FAIL |
| DATA/UANG — Marketing: pratinjau impor per baris = kenyataan (INV-F11) | FAIL |
| DATA/UANG — Marketing: berkas ekspor tidak boleh masuk toko yang salah (INV-F12) | FAIL |
| UANG/STOK — Dispatch ke buyer: satu rumus sisa kirim + hasil permak bisa dikirim (INV-F16) | FAIL |
| DOKUMEN — PDF rapi: 0 tumpang tindih + tabel penuh lebar halaman (INV-F17) | FAIL |
| STOK/UANG — Kirim material ke CMT menerbitkan MI + memotong stok + jurnal (INV-F18) | FAIL |
| PRODUK/STOK — Gudang: tombol buat MI, Buat Barcode, menu mati dilepas (INV-F19) | FAIL |
| LAYAR/UANG — Dashboard Marketing: ada pintunya + angka resmi dari SSOT siklus (INV-F20) | FAIL |
...
| DOKUMEN/NAVIGASI — Surat jalan satu daftar lintas sumber + pintu lama tak kosong (INV-F23) | FAIL |
| STOK/DOKUMEN — Arus keluar Cutting berdokumen, stok turun sekali (INV-F24) | PASS |
| DATA — Setelan penomoran dokumen benar-benar ditegakkan (INV-F25) | FAIL |
| DOKUMEN — Template PDF (kop/logo/kolom/TTD) benar-benar tercetak (INV-F26) | FAIL |
| STOK/DOKUMEN — Permak menaikkan sisa kirim · dispatch lanjutan · aksesoris BOM (INV-F27) | FAIL |
| UANG/DATA — Monitoring CMT: potongan sesuai order · scope PO · lacak pengganti (INV-F28) | FAIL |
| DATA — Sinkronisasi identitas barang Marketing ⇄ Gudang (INV-F29) | PASS |
| DATA/STOK — Identitas barang: warna·ukuran·OPSI tidak menabrak (INV-F30) | FAIL |
| ALUR/STOK — Retur pembeli Marketing → Retur Fisik gudang → stok (INV-F31) | FAIL |
| LAYAR/DOKUMEN — Tabel stok terbaca & kolom cetak bisa dipilih (INV-F32) | FAIL |
| DOKUMEN — Surat jalan CMT → DA bisa dicetak dari penerimaan FG (INV-F33) | FAIL |
| STOK — Alert stok hidup & satu definisi 'rendah' (INV-F34) | FAIL |
| STOK/PRODUKSI — Satuan gulungan, style dari master, harga dari pembelian (INV-F35) | PASS |
| UANG/PRODUKSI — HPP per potong dari pembelian & BOM mengisi cutting (INV-F36) | FAIL |
...
| UANG/DATA — Biaya jahit SPK, HPP batch FIFO, impor pintar, gaji host bulanan (INV-F39) | FAIL |
| DATA — KPI konten per konten/jenis/toko/KOL + rapor kreator mingguan (INV-F40) | FAIL |
| DATA — Impor master dari template Excel: dry-run, tolak cacat, idempoten (INV-F41) | FAIL |
| UANG — Pencairan marketplace: form Finance, COA akun toko, selisih bernama (INV-F42) | FAIL |
| UANG — Margin katalog: 0%/100% tidak dikarang saat HPP tak diketahui (INV-F43) | PASS |
| UANG — COGS pengiriman memakai biaya batch FIFO yang benar-benar keluar (INV-F44) | FAIL |
| LAYAR/UANG — impor pintar punya pintu + pencairan void tidak mengunci (INV-F45) | FAIL |

...
- Importir 08_MODEL menautkan kategori (104/104 model berkategori).

# [2026-09-12 #43] **MASTER PRODUK (Kategori·Varian·BOM·Ukuran·Warna) → PORTAL RnD · MATRIKS BOM PER WARNA**
- Portal Produksi → Master Produk hanya Model DA; Portal RnD → Master Produk (SSOT) menyunting resep. Tautan silang dua arah; deep-link lama diarahkan.
- Matriks BOM menampilkan semua warna tiap ukuran (472 resep terlihat), edit per warna.

# [2026-09-12 #42] **BOM MEMAKAI POTONGAN — HPP DARI KAIN YANG BENAR-BENAR DIPOTONG (INV-F46)**
- Keputusan owner: baris kain roll (kg/yard) di BOM diganti 1 pcs potongan `CUT-<MODEL>-<WARNA>-<UKURAN>`; biaya potongan = nilai kain dipotong ÷ pcs (Cutting, rata-rata bergerak). HPP jujur Rp 0 sebelum cutting nyata.
- SSOT `core/cut_panel_master.py`; cutting/importir/costing/BOM API/penjaga yatim disesuaikan; 472 BOM dikonversi; gate INV-F46 HIJAU.
- Master Produk Produksi disederhanakan + redesign; Master Lokasi pindah ke Gudang → Master Item (dengan Peran Gudang).

# [2026-09-12 #41] **CONTAINER BARU · IMPOR MASTER NYATA · 2 BUG KODE dari gate merah** (karantina bocor ke stok jual · kode pelanggan di luar kebijakan penomoran)

- Setup dari GitHub; master `MASTER_DATA_DA_PERBAIKAN.xlsx` diimpor (keputusan owner: DA-4401 = Rp 85.999; BOM DA-2101 465 kg → 0,465 kg;
  tarif jahit dibiarkan kosong). Angka: BOM 472 · katalog 640 · user 25 · karyawan 25 (+25 profil gaji — penghitung dibetulkan).
- P0 selisih CMT→DA/DA→buyer diverifikasi ulang: 43/43 · 12/12 · INV-13..18 hijau (2 asersi uji basi dibetulkan).
- BUG-1 `core/catalog_stock.blocked_location_ids`: lokasi `storage_role=karantina` (GD-L1-QC) tidak diblokir ⇒ stok rusak ikut stok jual. FIX.
- BUG-2 `routes/sales_direct.create_customer`: kode pelanggan di luar katalog penomoran. FIX: registry `rahaza_customers.code` + `issue_number` + `DocNumberField`.
- Gate dibetulkan agar bebas data demo: verify_rnd_invariants · verify_jembatan_retur · verify_marketing_cycle · gr_common.test_doc_number (mode manual sementara + restore).
- Rincian: `memory/PRD.md` bagian SESI 2026-09-12.
...
* **Bonus temuan:** berkas uji yang sama mematok `BASE_URL` ke preview container lama
  (`https://backend-test-suite-6.preview.emergentagent.com`) yang **sudah mati** ⇒ menguji host salah.

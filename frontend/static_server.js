...
function spawnLogged(cmd, args, out) {
  return spawn(cmd, args, { cwd: __dirname, detached: true, stdio: ["ignore", out, out] });
}
...
function ensureBuild(reason) {
  ...
  try {
    const child = spawnLogged("nice", ["-n", "19", "yarn", "build"], out);
    child.on("exit", (code) => {
      autoBuilding = false;
      try { fs.unlinkSync(BUILD_LOCK); } catch (_) {}
      console.log(`[static_server] auto-build selesai (exit ${code}).`);
      // Gagal sekali (mis. node_modules belum lengkap saat pod bangun) → yarn install lalu coba 1× lagi.
      if (code !== 0 && !buildRetried && !fs.existsSync(path.join(ROOT, "index.html"))) {
        buildRetried = true;
        console.log("[static_server] auto-build gagal → yarn install + retry…");
        const inst = spawnLogged("nice", ["-n", "19", "yarn", "install", "--frozen-lockfile"], out);
        inst.on("exit", () => ensureBuild("retry setelah yarn install"));
        inst.on("error", () => ensureBuild("retry tanpa install"));
        inst.unref();
      }
    });
    child.on("error", (e) => {

...
export function isChunkLoadError(error) {
  ...
  return error?.name === 'ChunkLoadError' || /Loading (CSS )?chunk [\w-]+ failed/i.test(msg) || /Failed to fetch dynamically imported module/i.test(msg);
}
...
export function reloadOnceForNewBuild() {
  try {
    ...
    if (Date.now() - last < 60_000) return false; // sudah dicoba <1 menit lalu → biarkan boundary tampil
...
class ErrorBoundary extends React.Component {
  ...
  static getDerivedStateFromError(error) {
    // ChunkLoadError = bundle di server sudah berganti (rebuild) sementara tab masih memegang
    // index.html lama → muat ulang sekali secara otomatis, jangan tampilkan kartu error.
    if (isChunkLoadError(error) && reloadOnceForNewBuild()) return { hasError: false };

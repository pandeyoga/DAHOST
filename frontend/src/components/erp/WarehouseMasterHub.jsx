...
import { Scale, Archive, RefreshCw, Loader2, BellRing, MapPin } from 'lucide-react';
...
import RahazaLocationsModule from './RahazaLocationsModule'; // 2026-09-12: pindah dari Produksi → Master Produk → Lokasi Kerja
...
export default function WarehouseMasterHub({ token }) {
  const [activeTab, setActiveTab]       = useState(() => {
    ...
    try {
      ...
      if (t) {
        ...
        if (['material', 'fg', 'thresholds', 'locations'].includes(t)) return t;
      }
    } catch (e) { /* noop */ }
    return 'material';
  });
  ...
  useEffect(() => {
    ...
    fetch('/api/rahaza/locations', { headers: h })
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (d) setLocCount(Array.isArray(d) ? d.length : (d?.total ?? d?.items?.length ?? null)); })
      .catch(() => {});
    ...
    <div className="h-full" data-testid="warehouse-master-hub">
      ...
      <div className="px-4 md:px-6 py-4 border-b bg-background">
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Master data Bahan, Aksesoris, Produk Jadi, dan Lokasi Gudang dalam satu tampilan terpadu
        </p>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
        ...
        <TabsContent value="locations" className="flex-1 overflow-auto m-0 p-4 md:p-6">

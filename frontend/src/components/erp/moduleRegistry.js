...
const EnginePOModule             = lazyRetry(() => import('./engine/ProductionPOModule'));
const EngineJobsModule           = lazyRetry(() => import('./engine/WorkOrderModule'));
const EngineProgressModule       = lazyRetry(() => import('./engine/ProductionProgressModule'));
const EngineReturnModule         = lazyRetry(() => import('./engine/ProductionReturnModule'));
const EngineBuyerShipmentModule  = lazyRetry(() => import('./engine/BuyerShipmentModule'));
const EngineVendorShipmentModule = lazyRetry(() => import('./engine/VendorShipmentModule'));
const EngineVarianceModule       = lazyRetry(() => import('./engine/OverproductionModule'));
const EngineVarianceReport       = lazyRetry(() => import('./engine/BuyerReceiptVarianceReport'));
const EngineMonitoringModule     = lazyRetry(() => import('./engine/ProductionMonitoringModule'));
const EngineSerialModule         = lazyRetry(() => import('./engine/SerialTrackingModule'));
const EngineDefectReportsModule  = lazyRetry(() => import('./engine/MaterialDefectReportsModule'));
// Phase B (2026-07-16) — DA-admin proses FG dari CMT (cmt_receipts SSOT).
const EngineDAReceiveFromCMT     = lazyRetry(() => import('./engine/DAReceiveFromCMTModule'));
// Phase C (2026-07-17) — PO Closure (auto-complete 100% + close-short + credit note).
const EnginePOClosure            = lazyRetry(() => import('./engine/POClosureModule'));

// Dashboards
const ManagementDashboard = lazyRetry(() => import('./ManagementDashboard'));
const WarehouseDashboard  = lazyRetry(() => import('./WarehouseDashboard'));
const FinanceDashboard    = lazyRetry(() => import('./FinanceDashboard'));
// Sprint 1.2: Replace placeholder with real HR Dashboard
const HRDashboard = lazyRetry(() => import('./HRDashboard'));
const HRApprovalInboxModule = lazyRetry(() => import('./HRApprovalInboxModule'));   // Phase 26 — P2 HR Approval Inbox
// Task 2.4: Multi-Level Approval Workflow
const MultiLevelApprovalModule = lazyRetry(() => import('./MultiLevelApprovalModule'));
// Phase 3.3B: Unified Approval Hub (Aggregator)
const UnifiedApprovalHub = lazyRetry(() => import('./UnifiedApprovalHub'));
// Task 1.2: Shift Management System
const HRShiftManagementModule = lazyRetry(() => import('./HRShiftManagementModule'));
// Announcement Management (HR)
const AnnouncementModule = lazyRetry(() => import('./AnnouncementModule'));
...
const BuyersModule          = lazyRetry(() => import('./BuyersModule'));
const ReportsModule         = lazyRetry(() => import('./ReportsModule'));
...
const ActivityLogModule     = lazyRetry(() => import('./ActivityLogModule'));
const CompanySettingsModule = lazyRetry(() => import('./CompanySettingsModule'));
...
// Warehouse
const ReceivingModule = lazyRetry(() => import('./ReceivingModule'));
const PutAwayModule   = lazyRetry(() => import('./PutAwayModule'));
...
const AccessoryModule  = lazyRetry(() => import('./AccessoryModule'));
// Portal Aksesoris dedicated modules (Session #11.21)
const AccessoriesDashboard = lazyRetry(() => import('./AccessoriesDashboard'));
const AccessoriesReports   = lazyRetry(() => import('./AccessoriesReports'));
...
// const EmployeeLoansModule = lazyRetry(() => import('./EmployeeLoansModule'));
const InventoryScrapModule = lazyRetry(() => import('./InventoryScrapModule'));

// Backup & Restore System - Added 2026-06-01
const BackupRestoreModule = lazyRetry(() => import('./BackupRestoreModule'));
const DocNumberingModule = lazyRetry(() => import('./DocNumberingModule'));

const WHReturnsModule  = lazyRetry(() => import('./WHReturnsModule'));
const QuarantineModule = lazyRetry(() => import('./QuarantineModule'));  // FASE 6: Karantina QC (INV-8)
const StockSchemaHealthModule = lazyRetry(() => import('./StockSchemaHealthModule'));  // FASE 6.6-A: kesehatan skema baris stok
...
// const CMTPackingModule = lazyRetry(() => import('./CMTPackingModule'));
...
const FinancialRecapModule     = lazyRetry(() => import('./FinancialRecapModule'));
const ThreeWayMatchModule      = lazyRetry(() => import('./ThreeWayMatchModule'));     // Phase 27 — 3-way match dashboard
const ARLifecycleModule        = lazyRetry(() => import('./ARLifecycleModule'));       // Phase 30 — AR 360° (Aging + Customer Statement)
const ApprovalModule           = lazyRetry(() => import('./ApprovalModule'));
// Finance · Bank Reconciliation (P1)
const BankReconciliation       = lazyRetry(() => import('./finance/BankReconciliation'));
...
const FinanceSettlementModule  = lazyRetry(() => import('./finance/FinanceSettlementModule'));
const CashFlowAI               = lazyRetry(() => import('./finance/CashFlowAI'));

// Produksi · Master Data Rajut (PT Rahaza)
const RahazaLocationsModule = lazyRetry(() => import('./RahazaLocationsModule'));
const RahazaProcessesModule = lazyRetry(() => import('./RahazaProcessesModule'));
const RahazaShiftsModule    = lazyRetry(() => import('./RahazaShiftsModule'));
const RahazaMachinesModule  = lazyRetry(() => import('./RahazaMachinesModule'));
const RahazaLinesModule     = lazyRetry(() => import('./RahazaLinesModule'));
const RahazaEmployeesModule = lazyRetry(() => import('./RahazaEmployeesModule'));
const RahazaModelsModule    = lazyRetry(() => import('./RahazaModelsModule'));
const RahazaSizesModule     = lazyRetry(() => import('./RahazaSizesModule'));
const ProductionDashboardModule    = lazyRetry(() => import('./ProductionDashboardModule'));
const ProductionControlTowerModule = lazyRetry(() => import('./ProductionControlTowerModule')); // Phase 28 — P2 Workflow Consolidation #3
// FASE IA-1 (2026-07-26) — AI produksi ASLI (dulu cuma tab di dashboard) & pintu Invoice CMT.
const AIInsightsModule             = lazyRetry(() => import('./AIInsightsModule'));
const ProductionCMTBillingModule   = lazyRetry(() => import('./ProductionCMTBillingModule'));
const RahazaOrdersModule           = lazyRetry(() => import('./RahazaOrdersModule'));
...
const RahazaAlertSettingsModule    = lazyRetry(() => import('./RahazaAlertSettingsModule'));
const RahazaMaterialsModule        = lazyRetry(() => import('./RahazaMaterialsModule'));
const RahazaStockModule            = lazyRetry(() => import('./RahazaStockModule'));
const RahazaMaterialIssueModule    = lazyRetry(() => import('./RahazaMaterialIssueModule'));
...
const RahazaCostCentersModule      = lazyRetry(() => import('./RahazaCostCentersModule'));
const RahazaARInvoicesModule       = lazyRetry(() => import('./RahazaARInvoicesModule'));
const FinChannelGLMappingModule    = lazyRetry(() => import('./FinChannelGLMappingModule'));
const RahazaCashAccountsModule     = lazyRetry(() => import('./RahazaCashAccountsModule'));
const RahazaExpensesModule         = lazyRetry(() => import('./RahazaExpensesModule'));
const RahazaHPPModule              = lazyRetry(() => import('./RahazaHPPModule'));
const ProductCostingModule         = lazyRetry(() => import('./costing/ProductCostingModule'));  // HPP per potong & per model
const SewingCostModule             = lazyRetry(() => import('./costing/SewingCostModule'));      // sesi #34: biaya jahit SPK + HPP batch FIFO
const ManagementOverviewModule     = lazyRetry(() => import('./ManagementOverviewModule'));
// O1.1 — RahazaShipmentsModule tidak lagi dipakai (prod-shipments → redirect wms-delivery-notes). Diarsip kelak.
// const RahazaShipmentsModule        = lazyRetry(() => import('./RahazaShipmentsModule'));
...
const RahazaTrialBalanceModule    = lazyRetry(() => import('./RahazaTrialBalanceModule'));
const RahazaPeriodsModule         = lazyRetry(() => import('./RahazaPeriodsModule'));
const RahazaGeneralLedgerModule   = lazyRetry(() => import('./RahazaGeneralLedgerModule'));
...
const MarketingARBridgeModule     = lazyRetry(() => import('./MarketingARBridgeModule'));
const AdminSetupPanelModule       = lazyRetry(() => import('./AdminSetupPanelModule'));

const RahazaPnLModule             = lazyRetry(() => import('./RahazaPnLModule'));
const RahazaHRReportsModule       = lazyRetry(() => import('./RahazaHRReportsModule'));
const RahazaBalanceSheetModule    = lazyRetry(() => import('./RahazaBalanceSheetModule'));
const RahazaJournalListModule     = lazyRetry(() => import('./RahazaJournalListModule'));
const RahazaAPAgingModule         = lazyRetry(() => import('./RahazaAPAgingModule'));

// Finance · Accounting Core (Phase F3)
const RahazaCashFlowModule        = lazyRetry(() => import('./RahazaCashFlowModule'));
const BudgetModule                = lazyRetry(() => import('./BudgetModule'));
const FixedAssetsModule           = lazyRetry(() => import('./FixedAssetsModule'));

// Session 13 — SLA, Management Tools, Smart Warehouse
const MaklonSLADashboard         = lazyRetry(() => import('./MaklonSLADashboard'));
const ManagementToolsModule      = lazyRetry(() => import('./ManagementToolsModule'));
const WarehouseSmartModule       = lazyRetry(() => import('./WarehouseSmartModule'));

// Phase 1/2 — Marketing Webhooks + Capacity Planning
const MarketingWebhooksModule    = lazyRetry(() => import('./marketing/MarketingWebhooksModule'));
const ProcurementRequestModule  = lazyRetry(() => import('./ProcurementRequestModule'));
...
const LiveSessionAnalyticsDashboard = lazyRetry(() => import('./marketing/LiveSessionAnalyticsDashboard'));
const PayrollDashboardModule      = lazyRetry(() => import('./PayrollDashboardModule'));
const ExecutiveReportModule       = lazyRetry(() => import('./ExecutiveReportModule'));
const ReportsHubModule            = lazyRetry(() => import('./ReportsHubModule'));

// Session 14 — AI Business Intelligence
const AIBusinessDashboard        = lazyRetry(() => import('./AIBusinessDashboard'));
...
const AssetManagementPortalLazy   = lazyRetry(() => import('./AssetManagementPortal'));
// BACKLOG-A — Hub konsolidasi (T3.3/T3.4/T3.5/T3.6/T3.9)
const FinanceJournalHub  = lazyRetry(() => import('./hubs/FinanceJournalHub'));
const MarketingAIHub     = lazyRetry(() => import('./hubs/MarketingAIHub'));
const HRAIHub            = lazyRetry(() => import('./hubs/HRAIHub'));
const MarketingLiveHub   = lazyRetry(() => import('./hubs/MarketingLiveHub'));
const RnDCostingHub      = lazyRetry(() => import('./hubs/RnDCostingHub'));
const WMSStockHub        = lazyRetry(() => import('./hubs/WMSStockHub'));   // RC-IA-warehouse-1 (Opsi A)
// PHASE B — 5 hub anti-overwhelm
const HRExpenseTravelHub        = lazyRetry(() => import('./hubs/HRExpenseTravelHub'));
const HRAttendanceHub           = lazyRetry(() => import('./hubs/HRAttendanceHub'));
const FinanceAccountingAdjustHub = lazyRetry(() => import('./hubs/FinanceAccountingAdjustHub'));
const RnDDesignHub              = lazyRetry(() => import('./hubs/RnDDesignHub'));
// IA v2.1 (pilot Produksi + Keuangan) — konsolidasi analitik/monitoring/master jadi hub bertab.
const ProductionAnalyticsHub      = lazyRetry(() => import('./hubs/ProductionAnalyticsHub'));
const ProductionMasterProcessHub  = lazyRetry(() => import('./hubs/ProductionMasterProcessHub'));
const ProductionMasterProductHub  = lazyRetry(() => import('./hubs/ProductionMasterProductHub'));
const RnDMasterProductHub         = lazyRetry(() => import('./hubs/RnDMasterProductHub')); // 2026-09-12: Kategori·Varian·BOM·Ukuran·Warna → RnD
const FinanceReportsHub           = lazyRetry(() => import('./hubs/FinanceReportsHub'));
const FinanceAccountingMasterHub  = lazyRetry(() => import('./hubs/FinanceAccountingMasterHub'));
// IA v2.1 fase-2/3 — hub konsolidasi Manajemen, SDM, Marketing.
const ManagementAccessHub  = lazyRetry(() => import('./hubs/ManagementAccessHub'));
const ManagementSystemHub  = lazyRetry(() => import('./hubs/ManagementSystemHub'));
const HRPayrollHub         = lazyRetry(() => import('./hubs/HRPayrollHub'));
const HRShiftHub           = lazyRetry(() => import('./hubs/HRShiftHub'));
const HRLeaveHub           = lazyRetry(() => import('./hubs/HRLeaveHub'));
const MarketingKOLHub      = lazyRetry(() => import('./hubs/MarketingKOLHub'));

// Staff Self-Service Portal
const SelfServicePortal           = lazyRetry(() => import('./SelfServicePortal'));

// Portal Saya — Self-Service HR + My Workspace
const PortalSayaDashboard         = lazyRetry(() => import('./PortalSayaDashboard'));
const PortalSayaProfile           = lazyRetry(() => import('./PortalSayaProfile'));
const PortalSayaCuti              = lazyRetry(() => import('./PortalSayaCuti'));
...
const PortalSayaAbsen             = lazyRetry(() => import('./PortalSayaAbsen'));
...
const HRAttendanceSessionsModule  = lazyRetry(() => import('./HRAttendanceSessionsModule'));
const PortalSayaPayslip           = lazyRetry(() => import('./PortalSayaPayslip'));
const PortalSayaTraining          = lazyRetry(() => import('./PortalSayaTraining'));
const PortalSayaNotifikasi        = lazyRetry(() => import('./PortalSayaNotifikasi'));
const WorkspaceHub                = lazyRetry(() => import('./WorkspaceHub'));

// Phase 12 — Kasbon & Pinjaman Karyawan
const KasbonStaffModule           = lazyRetry(() => import('./KasbonStaffModule'));
const HRKasbonModule              = lazyRetry(() => import('./HRKasbonModule'));
const FinanceKasbonModule         = lazyRetry(() => import('./FinanceKasbonModule'));

// Phase 8 — DA KPI System
const HRKPIModule                 = lazyRetry(() => import('./HRKPIModule'));
const KPIPortalModule             = lazyRetry(() => import('./KPIPortalModule'));

// Phase 8 — DA Employee Assets & Payroll Allowances
const HRAssetModule               = lazyRetry(() => import('./HRAssetModule'));
...
const HREmployeeModule            = lazyRetry(() => import('./HREmployeeModule'));
...
const HRAdminModule               = lazyRetry(() => import('./HRAdminModule'));
const HR360FeedbackModule         = lazyRetry(() => import('./HR360FeedbackModule'));

// Phase 7.9 — WMS Pick List Generator
const WMSPickListModule           = lazyRetry(() => import('./WMSPickListModule'));
// WMS P0/P1 Garment Features
const WMSFabricRollsModule        = lazyRetry(() => import('./WMSFabricRollsModule'));
const WMSDeliveryNotesModule      = lazyRetry(() => import('./WMSDeliveryNotesModule'));
const WMSCMTDispatchesModule      = lazyRetry(() => import('./WMSCMTDispatchesModule'));
// FASE H-3 (2026-08-16): menu "Buat Barcode" — label bahan & barang jadi.
const WMSBarcodeModule            = lazyRetry(() => import('./WMSBarcodeModule'));
const WMSOpnameScanModule         = lazyRetry(() => import('./WMSOpnameScanModule'));  // Fase 4: opname scan-driven (SSOT); Fase 5: menggantikan WMSOpnameEnhancedModule (dihapus)

// ─── Portal Maklon (Fase 3) ───────────────────────────────────────────────────
const MaklonDashboard = lazyRetry(() => import('./MaklonDashboard'));
const MaklonClientManagement = lazyRetry(() => import('./MaklonClientManagement'));
// Phase M1: Buyer Catalog (master artikel buyer Maklon, terpisah dari DA Product Master)
const MaklonBuyerCatalogModule = lazyRetry(() => import('./MaklonBuyerCatalogModule'));
...
// Fase 3B: Sample & QC
const MaklonSampleManagement = lazyRetry(() => import('./MaklonSampleManagement'));
const MaklonQCTracking = lazyRetry(() => import('./MaklonQCTracking'));
const MaklonProductionTracking = lazyRetry(() => import('./MaklonProductionTracking'));
// Fase 3C: Billing & HPP
const MaklonBillingModule = lazyRetry(() => import('./MaklonBillingModule'));
const MaklonHppModule = lazyRetry(() => import('./MaklonHppModule'));
const MaklonSystemConfigModule = lazyRetry(() => import('./MaklonSystemConfigModule'));
const NotificationCenterModule = lazyRetry(() => import('./NotificationCenterModule'));
...
const MaklonPO360Module = lazyRetry(() => import('./MaklonPO360Module'));    // 360° unified view per PO (Phase 25)
const CMTPermakModule   = lazyRetry(() => import('./CMTPermakModule'));      // Permak/Rework (Maklon Revamp M1)
const CMTMonitorModule  = lazyRetry(() => import('./CMTMonitorModule'));     // Monitoring CMT: Dashboard Owner + Kejar (Fase 2)
...
// const CMTProgressModule = lazyRetry(() => import('./CMTProgressModule'));    // CMT progress + DO
// const CMTLifecycleModule = lazyRetry(() => import('./CMTLifecycleModule'));   // Phase 29 — Vendor lifecycle dashboard
// FASE 7: VendorPortalModule (portal vendor LAMA, model vendor_jobs) dipensiunkan.
const VendorCMTEnginePortal = lazyRetry(() => import('../vendor-cmt/VendorCMTEnginePortal'));  // portal vendor SSOT (/api/production-*)
const VendorAccountsAdminModule = lazyRetry(() => import('./VendorAccountsAdminModule'));  // Session #11.21 — Admin kelola vendor
...
const CMTOverridePortalModule = lazyRetry(() => import('./CMTOverridePortalModule'));
...
const MarketingDashboard = lazyRetry(() => import('./MarketingDashboard'));
const AccountManagementModule = lazyRetry(() => import('./AccountManagementModule'));
const AccountBulkReviewModule = lazyRetry(() => import('./marketing/AccountBulkReviewModule'));
const SalesDataEntryModule = lazyRetry(() => import('./SalesDataEntryModule'));
const TaskManagementModule = lazyRetry(() => import('./TaskManagementModule'));
const ApprovalInboxModule = lazyRetry(() => import('./ApprovalInboxModule'));
const TaskTemplatesModule = lazyRetry(() => import('./TaskTemplatesModule'));
// Consolidation #9 & #10 — Hub modules (2026-05-23)
const MarketingAfterSalesHub  = lazyRetry(() => import('./MarketingAfterSalesHub'));   // Komplain + Returns + Log
const MarketingTaskHubModule   = lazyRetry(() => import('./MarketingTaskHubModule'));  // Kanban + Approval + Templates
// Consolidation #3 & #13 — Warehouse + Production master hubs (2026-05-23)
const WarehouseMasterHub        = lazyRetry(() => import('./WarehouseMasterHub'));         // Material + FG Master
// SESI #33 — Daftar Belanja Mingguan (usul beli dari ambang → PR) & Riwayat Harga Barang
const WeeklyShoppingListModule  = lazyRetry(() => import('./WeeklyShoppingListModule'));
const MaterialCostHistoryModule = lazyRetry(() => import('./MaterialCostHistoryModule'));
const ProductionWorkspaceMaster = lazyRetry(() => import('./ProductionWorkspaceMaster')); // Lokasi + Lini + Mesin + Shift
// Consolidation #8 & #14 — Marketing Reports + HR Performance hubs (2026-05-23)
const MarketingReportsHub   = lazyRetry(() => import('./MarketingReportsHub'));   // Overview + Sales + Ads + Daily + Monthly
const MarketingSettlementsView = lazyRetry(() => import('./marketing/MarketingSettlementsView')); // sesi #34: pencairan marketplace (lihat saja)
const HRPerformanceHub      = lazyRetry(() => import('./HRPerformanceHub'));      // KPI + Annual Review + 360° Feedback
const ImportCenterModule = lazyRetry(() => import('./ImportCenterModule'));
const CatalogManagementModule = lazyRetry(() => import('./CatalogManagementModule'));
...
const SkuBridgeModule = lazyRetry(() => import('./SkuBridgeModule'));
const SyncAuditModule = lazyRetry(() => import('./SyncAuditModule'));
const AuditFindingsBoard = lazyRetry(() => import('./AuditFindingsBoard')); // Papan Temuan Audit 2026-09-23
const KOLCreatorModule = lazyRetry(() => import('./KOLCreatorModule'));

// Phase 2 Week 4-5: Orders & Complaints Management
const UnifiedOrdersDashboard = lazyRetry(() => import('./marketing/UnifiedOrdersDashboard'));
...
const FulfillmentMonitorModule = lazyRetry(() => import('./marketing/FulfillmentMonitorModule'));
const ComplaintsManagementModule = lazyRetry(() => import('./marketing/ComplaintsManagementModule'));

// Phase 3 Week 6-7: Account Health, Sales Performance, Ads, Live Sessions
const AccountHealthDashboard = lazyRetry(() => import('./marketing/AccountHealthDashboard'));
const SalesPerformanceDashboard = lazyRetry(() => import('./marketing/SalesPerformanceDashboard'));
const AdsPerformanceDashboard = lazyRetry(() => import('./marketing/AdsPerformanceDashboard'));
const LiveSessionModule = lazyRetry(() => import('./marketing/LiveSessionModule'));

// Phase 3 Week 8-10: Content Calendar, Discount Campaign, Product Launch
const ContentCalendarModule = lazyRetry(() => import('./marketing/ContentCalendarModule'));
const DiscountCampaignModule = lazyRetry(() => import('./marketing/DiscountCampaignModule'));
const ProductLaunchModule = lazyRetry(() => import('./marketing/ProductLaunchModule'));

// Phase 3 Week 11-12: Marketing Overview Dashboard + Integration Settings
const MarketingOverviewDashboard = lazyRetry(() => import('./marketing/MarketingOverviewDashboard'));
...
const MarketingIntegrationSettings = lazyRetry(() => import('./marketing/MarketingIntegrationSettings'));
...
const MarketingSchedulerModule = lazyRetry(() => import('./marketing/MarketingSchedulerModule'));

// Phase 3 Week 13: Fitur Internal (Rating/Review, Returns, Sample Delivery)
const RatingReviewModule = lazyRetry(() => import('./marketing/RatingReviewModule'));
const ReturnsRefundsModule = lazyRetry(() => import('./marketing/ReturnsRefundsModule'));
const SampleDeliveryModule = lazyRetry(() => import('./marketing/SampleDeliveryModule'));
// Session 28 — LiveHost Management (Phase 1-4)
const LiveHostModule = lazyRetry(() => import('./marketing/LiveHostModule'));
// Session 28 — Marketing PIC Reports & Targets
const AccountTargetsModule = lazyRetry(() => import('./marketing/AccountTargetsModule'));
// F6.5 (sesi #9) — jejak perubahan marketing (layar "siapa mengubah apa").
const MarketingChangeLogModule = lazyRetry(() => import('./marketing/MarketingChangeLogModule'));
const DailyReportModule    = lazyRetry(() => import('./marketing/DailyReportModule'));
const MonthlyReportModule  = lazyRetry(() => import('./marketing/MonthlyReportModule'));

// DEPRECATED (Phase 3.3A): toko-dashboard-legacy dan toko-dashboard-classic sekarang redirect ke toko-dashboard SSOT
// const TokoDashboard = lazyRetry(() => import('./TokoDashboard'));
// const TokoDashboardModule = lazyRetry(() => import('./TokoDashboardModule'));
const TokoChannelManagerModule = lazyRetry(() => import('./TokoChannelManagerModule'));
// Phase 5B: Orders, Pricing/Flashsale, KOL, CS/Returns
const TokoOrdersModule = lazyRetry(() => import('./TokoOrdersModule'));
const FulfillmentModule = lazyRetry(() => import('./FulfillmentModule'));  // Phase 6: Online Order Bridge
...
// const DOManagementModule = lazyRetry(() => import('./DOManagementModule'));  // Phase 2 Enhancement: DO System
const UnifiedInventoryModule = lazyRetry(() => import('./UnifiedInventoryModule'));  // Phase 2 Enhancement: Unified Inventory
const Phase7ReportingModule = lazyRetry(() => import('./Phase7ReportingModule'));  // Phase 7: Laporan & Dashboard
const TokoPricingFlashsaleModule = lazyRetry(() => import('./TokoPricingFlashsaleModule'));
...
const TokoCSReturnsModule = lazyRetry(() => import('./TokoCSReturnsModule'));

// ─── Fase 2: Cutting & CMT ────────────────────────────────────────────────────
// O1.2 — CMTManagementModule tidak lagi dipakai (prod-cmt → redirect vendor-admin).
// FASE 20 — SUDAH DIARSIP ke `_archive/`.
// const CMTManagementModule  = lazyRetry(() => import('./CMTManagementModule'));

// ─── Phase 6 — HRIS (Full) ───────────────────────────────────────────────────
const HRPerformanceModule = lazyRetry(() => import('./HRPerformanceModule'));
const HRLMSModule = lazyRetry(() => import('./HRLMSModule'));
const HROnboardingModule = lazyRetry(() => import('./HROnboardingModule'));
const HRATSModule = lazyRetry(() => import('./HRATSModule'));
const HROrgChartModule = lazyRetry(() => import('./HROrgChartModule'));

// ─── Phase 7 — RnD & Style Master ────────────────────────────────────────────
const RnDModule = lazyRetry(() => import('./RnDModule'));

// ─── Session 26 — Portal RnD (dedicated portal, 2026-05-15) ─────────────────
const RnDPortalDashboard      = lazyRetry(() => import('./RnDPortalDashboard'));
...
const RnDAnalyticsModule      = lazyRetry(() => import('./RnDAnalyticsModule'));
// Re-use existing tabs as standalone portal modules:
const RnDStylesTab    = lazyRetry(() => import('./RnDStylesTab'));
const RnDProductViewer = lazyRetry(() => import('./RnDProductViewer'));  // sesi #34: katalog produk final RnD + status SSOT
const RnDSamplesTab   = lazyRetry(() => import('./RnDSamplesTab'));
const RnDMaterialsTab = lazyRetry(() => import('./RnDMaterialsTab'));
...
const RnDStyleDetailPage = lazyRetry(() => import('./RnDStyleDetailPage'));
...
const AccessoryRequestInbox      = lazyRetry(() => import('./AccessoryRequestInbox'));
const CMTComponentRequestModule  = lazyRetry(() => import('./CMTComponentRequestModule'));

// Sprint 2.1 — Purchase Orders
const PurchaseOrderModule = lazyRetry(() => import('./PurchaseOrderModule'));
...
const RahazaLineBalancingModule = lazyRetry(() => import('./RahazaLineBalancingModule'));

// Phase 22B — Shift Handover, Material Reservation, Production Calendar
const RahazaShiftHandoverModule      = lazyRetry(() => import('./RahazaShiftHandoverModule'));
const RahazaProductionCalendarModule  = lazyRetry(() => import('./RahazaProductionCalendarModule'));
// Phase 23 — OEE Dashboard
// User Guide
const RahazaUserGuideModule = lazyRetry(() => import('./RahazaUserGuideModule'));
...
const RahazaModelsAndBOMModule  = lazyRetry(() => import('./RahazaModelsAndBOMModule'));
const RahazaMaterialRequirementsModule = lazyRetry(() => import('./RahazaMaterialRequirementsModule')); // Fase 5: MRP-lite
const IntegrationSettingsModule = lazyRetry(() => import('./IntegrationSettingsModule'));

// FG Inventory (Produk Jadi)
const RahazaFGInventoryModule   = lazyRetry(() => import('./RahazaFGInventoryModule'));

// Session 22 (Phase 4) — P1 GRN Quality Check + Supplier Scorecard
const SupplierScorecardModule   = lazyRetry(() => import('./SupplierScorecardModule'));
// ─── Portal Pengadaan (2026-08-06) — procurement dilepas dari Gudang/Keuangan ──
const ProcurementDashboardModule = lazyRetry(() => import('./procurement/ProcurementDashboardModule'));
const SupplierMasterModule       = lazyRetry(() => import('./procurement/SupplierMasterModule'));
// ─── Portal Penjualan (2026-09) — penjualan langsung dari stok FG ────────────
const SalesDashboardModule       = lazyRetry(() => import('./sales/SalesDashboardModule'));
const SalesCustomersModule       = lazyRetry(() => import('./sales/SalesCustomersModule'));
const DirectSalesModule          = lazyRetry(() => import('./sales/DirectSalesModule'));
const SalesReportModule          = lazyRetry(() => import('./sales/SalesReportModule'));
const ProcurementSpendModule     = lazyRetry(() => import('./procurement/ProcurementSpendModule'));
const SupplierInvoiceModule      = lazyRetry(() => import('./procurement/SupplierInvoiceModule'));

// WMS (Phase 7 — Warehouse Management System w/ Scanner)
const WMSModule                 = lazyRetry(() => import('./WMSModule'));

// Production Automation (Phase 4)

// Session 15 — HR AI & Portal Saya Extensions
const HRResumeScreeningModule = lazyRetry(() => import('./hr/HRResumeScreeningModule'));
const HRAttritionModule = lazyRetry(() => import('./hr/HRAttritionModule'));
const HRCoachingModule = lazyRetry(() => import('./hr/HRCoachingModule'));
const MyDocumentsModule = lazyRetry(() => import('./portal/MyDocumentsModule'));
const MyAnnualReviewModule = lazyRetry(() => import('./portal/MyAnnualReviewModule'));
const PeerFeedbackModule = lazyRetry(() => import('./portal/PeerFeedbackModule'));

// Session 17 Batch 1 — HR/SDM Features (P2-11, P2-12, P2-16)
const ShiftSchedulerModule = lazyRetry(() => import('./hr/ShiftSchedulerModule'));
const JobBoardModule = lazyRetry(() => import('./hr/JobBoardModule'));
const CareerCoachModule = lazyRetry(() => import('./portal/CareerCoachModule'));
...
const OKRTrackerModule = lazyRetry(() => import('./OKRTrackerModule'));
const PredictiveMaintenanceModule = lazyRetry(() => import('./PredictiveMaintenanceModule'));
const MaklonAIQuoteModule = lazyRetry(() => import('./MaklonAIQuoteModule'));

// Session 19 — E-3: AI Usage Monitor (Admin only)
const AIUsageMonitorModule = lazyRetry(() => import('./AIUsageMonitorModule'));
...
const FinanceExpenseHub = lazyRetry(() => import('./hubs/FinanceExpenseHub'));  // RC-FLOW-UX de-dup: 2 pintu expense → 1 hub ber-tab
const EmployeeExpenseGLMappingModule = lazyRetry(() => import('./EmployeeExpenseGLMappingModule'));
const EmployeeExpenseCategoryMasterModule = lazyRetry(() => import('./EmployeeExpenseCategoryMasterModule')); // Phase 5D
const PettyCashModule      = lazyRetry(() => import('./PettyCashModule'));      // Phase 6B
const BankTransferModule   = lazyRetry(() => import('./BankTransferModule'));   // Phase 6C
...
export const MODULE_REGISTRY = {
  ...
  // 2026-09-12 — laporan ala Excel akuntan (deep-link → tab hub)
  'fin-worksheet':         makeRedirect('fin-reports-hub', 'worksheet'),
  'fin-pnl-monthly':       makeRedirect('fin-reports-hub', 'pnl12'),
  'fin-balance-sheet-monthly': makeRedirect('fin-reports-hub', 'bs12'),
  'fin-opening-balance':   makeRedirect('fin-accounting-master-hub', 'opening'),
  ...
  'audit-findings':         AuditFindingsBoard,
  ...
  'rnd-master-product-hub': RnDMasterProductHub,   // 2026-09-12 (owner): SSOT resep produk pindah dari Produksi
  'rnd-bom':        makeRedirect('rnd-master-product-hub', 'bom'),
  'rnd-sizes':      makeRedirect('rnd-master-product-hub', 'sizes'),
  'rnd-colors':     makeRedirect('rnd-master-product-hub', 'colors'),
  'rnd-categories': makeRedirect('rnd-master-product-hub', 'categories'),
  'rnd-sku-variants': makeRedirect('rnd-master-product-hub', 'variants'),
  ...
  'collaboration':            lazyRetry(() => import('./CollaborationPortal')),
  'collab-workspace':         lazyRetry(() => import('./WorkspacePortal')),  // Spreadsheet Workspace
  'collab-communication':     lazyRetry(() => import('./CommunicationHubPortal')),  // Direct access
  ...
  'cutting-dashboard': lazyRetry(() => import('./cutting/CuttingDashboard')),
  'cutting-orders':    lazyRetry(() => import('./cutting/CuttingOrdersModule')),
  'cutting-panels':    lazyRetry(() => import('./cutting/CuttingPanelsModule')),
  // Pengaturan notifikasi berkategori (Portal Administrasi Sistem)
  'sys-notif-config':  lazyRetry(() => import('./NotificationSettingsModule')),
  'sys-rbac-audit':    lazyRetry(() => import('./RbacAuditModule')),
  ...
  'prod-models':             makeRedirect('prod-master-product-hub', 'models'),
  // 2026-09-12 (owner): BOM & Ukuran kini di Portal RnD → Master Produk
  'prod-bom':                makeRedirect('rnd-master-product-hub', 'bom'),
  'prod-sizes':              makeRedirect('rnd-master-product-hub', 'sizes'),
};

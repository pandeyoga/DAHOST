    ...
    <div className="space-y-5" data-testid="fill-page">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        <GlassCard className="p-5 space-y-3"><div className="text-[10px] uppercase text-muted-foreground font-semibold">Langkah 1</div><h3 className="font-semibold text-sm">Unduh berkas isian</h3>
          <Button onClick={dlGap} className="h-9 w-full" data-testid="fill-download-gap"><Download className="w-3.5 h-3.5 mr-1.5" />{file ? 'Data yang Perlu Diisi — SISA dari berkas ini' : 'Data yang Perlu Diisi (1 berkas)'}</Button>
          <p className="text-[11px] text-muted-foreground">{file ? 'Dibuat dari berkas yang sedang diunggah: hanya kelompok BOM yang dilewati (ditulis ulang utuh + kolom masalah/tindakan), sheet VARIAN_BARU untuk model tanpa SKU, MATERIAL harga 0 / isi kemasan.' : 'Dibuat dari kondisi DB saat ini — setelah BOM diterapkan, angkanya sudah yang terbaru.'}</p>

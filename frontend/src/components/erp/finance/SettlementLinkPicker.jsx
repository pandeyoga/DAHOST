/**
 * SettlementLinkPicker — pilih PENARIKAN saldo platform (tahap 2) untuk satu baris mutasi bank.
...
export function SettlementLinkPicker({ sessionId, txn, headers, onLink }) {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/api/finance/bank-recon/sessions/${sessionId}/transactions/${txn.id}/settlement-candidates`, { headers })
      ...
      .catch((e) => { if (alive) setError(e.response?.data?.detail || 'Gagal memuat penarikan.'); })
  ...
  return (
    <div className="mt-3 pl-4 border-l-2 border-emerald-500/40" data-testid={`settlement-picker-${txn.id}`}>
      <p className="text-xs font-semibold mb-1 text-emerald-700 flex items-center gap-1.5">
        <Banknote className="w-3.5 h-3.5" /> Tautkan ke Penarikan Saldo Marketplace
        <span className="font-normal text-muted-foreground">
          · {exact} penarikan bernominal sama persis
        </span>
      </p>
      {txn.type !== 'debit' ? (
        <p className="text-xs text-amber-700" data-testid="settlement-picker-not-debit">
          Baris ini uang keluar — penarikan saldo marketplace selalu uang masuk.
        </p>
      ) : null}
      {error ? (
      ...
      {loading ? (
        ...
        <p className="text-xs text-muted-foreground" data-testid="settlement-picker-empty">
          Tidak ada penarikan saldo yang belum tertaut. Catat dulu di Saldo & Pencairan Marketplace → Tahap 2 Penarikan Saldo ke Bank.
        </p>
      ) : (
        <div className="max-h-56 overflow-y-auto space-y-1">
          {items.map((s) => (
            <button key={s.id} disabled={!s.amount_match || busy === s.id || txn.type !== 'debit'}
              ...
              title={s.amount_match ? 'Tautkan' : `Nominal berbeda ${fmt(s.amount_diff)} — koreksi penarikan dulu`}
              ...
              <div className="min-w-0">
                ...
                <div className="text-muted-foreground">
                  masuk bank {s.settlement_date} · {s.days_apart != null ? `${s.days_apart} hari dari mutasi` : 'tanggal tak terbaca'}

...
import { BankSelect } from './PayoutBankLinks';
...
const EMPTY = { account_id: '', withdrawal_date: '', amount: '', reference: '', notes: '', cash_code: '' };

export function PlatformWithdrawalsPanel({ accounts, accountId, balances, banks = [], onChanged }) {
  ...
  const accCash = (id) => accounts.find((a) => a.id === id)?.coa_cash_code || '';
  const bankName = (code) => banks.find((b) => b.code === code)?.name || '';
  ...
  const openCreate = () => {
    setForm({ ...EMPTY, account_id: accountId || '', cash_code: accCash(accountId) }); setEditId(''); setFormOpen(true);
  };
  const openEdit = (r) => {
    setForm({ account_id: r.account_id, withdrawal_date: r.withdrawal_date || '', amount: String(r.amount ?? ''),
      reference: r.reference || '', notes: r.notes || '', cash_code: r.cash_code || accCash(r.account_id) });
  ...
  const save = async () => {
    ...
    if (!form.cash_code) { toast.error('Pilih rekening bank tujuan.'); return; }
    setBusy('save');
    try {
  ...
  return (
    <GlassCard className="p-4 space-y-3" data-testid="platform-withdrawals-panel">
      ...
      <WithdrawalImportPanel result={importRes} file={importFile} accounts={accounts} accountId={importAccount} banks={banks}
        ...
        <div className="rounded-lg border border-foreground/10 p-3 space-y-3" data-testid="wd-form">
          ...
          <div className="grid md:grid-cols-3 xl:grid-cols-6 gap-3">
            <label className="text-xs space-y-1">
              ...
              <select data-testid="wd-input-account" value={form.account_id}
                onChange={(e) => setForm({ ...form, account_id: e.target.value, cash_code: accCash(e.target.value) || form.cash_code })}
            ...
            <label className="text-xs space-y-1">
              <span className="text-foreground/60">Rekening bank tujuan</span>
              <BankSelect banks={banks} value={form.cash_code} testId="wd-input-bank"
                onChange={(v) => setForm({ ...form, cash_code: v })} />
            </label>
            <label className="text-xs space-y-1">
          ...
          {form.account_id ? (
            <div className={`text-xs rounded-lg px-3 py-2 ${available != null && parseFloat(form.amount || 0) > available + 0.01
              ...
              {form.cash_code ? <> · masuk ke rekening <span className="font-mono">{form.cash_code}</span> {bankName(form.cash_code)}</> : <> · <span className="text-amber-600">rekening bank tujuan belum dipilih</span></>}
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b border-foreground/5" data-testid={`wd-row-${r.reference}`}>
                  ...
                  <td className="py-2 px-3 text-[10px]" data-testid={`wd-bank-${r.reference}`}>
                    <div className="text-xs"><span className="font-mono">{r.cash_code || accCash(r.account_id) || '—'}</span> {bankName(r.cash_code || accCash(r.account_id))}</div>
                    {r.bank_txn_id

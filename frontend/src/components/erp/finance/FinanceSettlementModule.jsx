/**
 * FinanceSettlementModule — **Saldo & Pencairan Marketplace** di Portal Keuangan.
 *
 * DUA TAHAP (keputusan pemilik): omzet input Marketing TIDAK pernah masuk Finance.
 * Yang masuk Finance hanya:
 *  1. DANA DILEPAS platform ke saldo toko (laporan Penghasilan Shopee / Settlement
 *     TikTok) → Dr Piutang Toko 1-1303-xxx / Dr potongan / Cr Pendapatan toko (bruto).
 *     Saldo 1-1303-xxx = saldo yang masih bisa dicairkan.
 *  2. PENARIKAN saldo ke rekening bank → Dr Rekening Pencairan / Cr Piutang Toko.
 *     Dokumen inilah yang dicocokkan ke mutasi bank.
 *
 * Form ada di FINANCE (sesi #37): Marketing hanya melihat (`MarketingSettlementsView`).
 * `net_payout` tahap 1 DIISI TANGAN dari laporan platform — server menghitung
 * "seharusnya berapa" lalu menampilkan SELISIH-nya; selisih ≠ 0 ⇒ jurnal ditolak.
 */
...
import { PlatformBalanceCards, usePlatformBalances } from './PlatformBalanceCards';
import { PlatformWithdrawalsPanel } from './PlatformWithdrawalsPanel';
import { PayoutBankLinks, useBankOptions } from './PayoutBankLinks';
...
export default function FinanceSettlementModule() {
  ...
  const { data: balances } = usePlatformBalances(refreshKey);
  ...
  const rememberMapping = async (account_id) => {
    ...
    try {
      ...
      toast.warning(`Laporan tersimpan, tetapi pemetaan kolom gagal diingat: ${e.message}`);
    }
  };
  const openEdit = (r) => {
    const f = { ...EMPTY };
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success(d.data?.math_verified
        ? 'Dana dilepas tersimpan dan angkanya seimbang — saldo platform toko bertambah.'
  ...
  const act = async (r, kind) => {
    ...
    try {
      if (kind === 'journal') {
        ...
        toast.success('Laporan dana dilepas dihapus.');
      }
      await load();
    } catch (e) {
      toast.error(e.message);
  ...
  return (
    <div className="space-y-4" data-testid="finance-settlement-module">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-lg font-semibold flex items-center gap-2">
            <Banknote className="w-5 h-5" /> Saldo & Pencairan Marketplace
          </h2>
          <p className="text-xs text-foreground/60 mt-0.5 max-w-3xl">
            <b>Tahap 1 — Dana dilepas platform ke saldo toko</b> (laporan Penghasilan Shopee / Settlement
            TikTok): omzet bruto dikurangi potongan → masuk <b>saldo platform (piutang toko 1-1303-xxx)</b>,
            bukan bank. <b>Tahap 2 — Penarikan</b> saldo itu ke rekening bank ada di panel bawah.
            Omzet dari input sales Marketing tidak pernah masuk ke sini.
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button data-testid="fin-settlement-new" onClick={openCreate}
            ...
            <Plus className="w-4 h-4" /> Catat dana dilepas
          </button>
        </div>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Kpi testId="fin-settlement-kpi-net" icon={Banknote} label="Total masuk saldo platform"
          value={rp(summary?.net_payout || 0)} tone="good"
          hint={`${rows.length} laporan dana dilepas${range.from || range.to ? ` · ${range.from || '…'} → ${range.to || '…'}` : ''}`} />
      ...
      {/* ── SALDO PLATFORM PER TOKO + TAHAP 2 (PENARIKAN) ── */}
      <PlatformBalanceCards refreshKey={refreshKey} />
      <PayoutBankLinks accounts={accounts} banks={banks} onChanged={loadAccounts} />
      <PlatformWithdrawalsPanel accounts={accounts} accountId={accountId} banks={banks}
        balances={balances?.data} onChanged={() => setRefreshKey((k) => k + 1)} />
        ...
        <GlassCard className="p-4 space-y-3" data-testid="fin-settlement-form">
          <div className="flex items-center justify-between">
            <h3 className="font-medium text-sm">
              {editId ? 'Koreksi laporan dana dilepas' : 'Tahap 1 — Catat dana dilepas ke saldo platform'}
            </h3>
          ...
          <div className="grid md:grid-cols-4 gap-3">
            ...
            <label className="text-xs space-y-1">
              <span className="text-foreground/60">No. laporan / settlement (dari platform)</span>
            ...
            <label className="text-xs space-y-1">
              <span className="text-foreground/60">Tanggal dana dilepas</span>
          ...
          <div className="grid md:grid-cols-3 gap-3">
            <label className="text-xs space-y-1">
              <span className="text-foreground/60">
                Dana bersih masuk saldo platform <b>menurut laporan platform</b>
              </span>
        ...
        <GlassCard className="p-4 space-y-2" data-testid="fin-settlement-recon">
          ...
          <div className="grid md:grid-cols-3 gap-3 text-xs">
            <div className="rounded-lg bg-foreground/5 p-3">
              ...
              <div>masuk saldo <b>{rp(recon.settlement?.net_payout || 0)}</b></div>
      ...
      <GlassCard className="p-0 overflow-hidden">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm text-foreground/60 px-6" data-testid="fin-settlement-empty">
            Belum ada laporan dana dilepas tercatat{accountId ? ' untuk toko ini' : ''}{range.from || range.to ? ' pada periode ini' : ''}.
            <div className="mt-1 text-xs text-foreground/50">
              Buka laporan Penghasilan (Shopee) / Settlement (TikTok), lalu tekan “Catat dana dilepas” atau “Impor laporan”.
            </div>
          </div>
        ) : (
            ...
            <table className="w-full text-sm" data-testid="fin-settlement-table">
              <thead>
                <tr className="text-xs uppercase tracking-wide text-foreground/50 border-b border-foreground/10">
                  ...
                  <th className="text-left py-2 px-3">No. Laporan</th>
                  ...
                  <th className="text-right py-2 px-3">Masuk Saldo</th>

...
export default function Login({ onLogin }) {
  ...
  const [pending, setPending] = useState(null); // login sukses tapi wajib ganti sandi

  const finish = (data) => onLogin(data.token, {
    ...data.user,
  ...
  if (pending) {
    return (
      <ForcePasswordChange token={pending.token} email={pending.user?.email} oldPassword={password}
        onDone={() => finish({ ...pending, user: { ...pending.user, must_change_password: false } })} />
    );
  }

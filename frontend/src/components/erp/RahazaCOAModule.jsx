  ...
  return (
    <GlassCard className="p-5" data-testid="coa-editor">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div>
          <label className="text-[10px] uppercase text-muted-foreground font-semibold">Kelompok Arus Kas</label>
          <select
            value={form.cash_flow_group || ''}
            onChange={e => handle('cash_flow_group', e.target.value)}
            className="w-full h-9 rounded-[var(--radius-sm)] bg-[var(--card-surface)] border border-[var(--glass-border)] px-2 text-sm text-foreground backdrop-blur-sm"
            data-testid="coa-input-cfgroup"
          >
            <option value="">— bawaan dari kode akun —</option>
            <option value="OPR">OPR · Operasi</option>
            <option value="INV">INV · Investasi</option>
            <option value="PND">PND · Pendanaan</option>
          </select>
          <p className="text-[11px] text-muted-foreground mt-1">Dipakai Laporan Arus Kas “Per Akun”: akun lawan jurnal kas dikelompokkan lewat tag ini.</p>
        </div>
...
function FlatAccountRow({ acc, onEdit, onDelete }) {
  return (
    <tr className="border-b border-[var(--glass-border)] hover:bg-foreground/5">
      ...
      <td className="py-2 px-2 text-[10px] font-mono text-muted-foreground" data-testid={`coa-cfg-${acc.code}`}>{acc.cash_flow_group || '-'}</td>
      <td className="py-2 px-2">
      ...
      <table className="w-full text-sm">
        <thead className="sticky top-0 bg-[var(--card-surface)] backdrop-blur-sm">
          <tr className="text-left text-[10px] uppercase text-muted-foreground border-b border-[var(--glass-border)]">
            ...
            <th className="py-2 px-2">Arus Kas</th>

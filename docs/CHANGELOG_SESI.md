# CHANGELOG SESI (arsip blok status README) — CV. Dewi Aditya ERP

> Dipindahkan dari `README.md` pada 2026-09-22 (FASE 5 / T-28 `PLAN_PERBAIKAN_AUDIT.md`). Isi TIDAK diubah; ini arsip sejarah.
> Log sesi terkini: `memory/PRD.md` (paling atas). Status rencana perbaikan: `memory/PLAN_PERBAIKAN_AUDIT.md` §F.

## A. Blok status kronologis (2026-05 → 2026-09)
...
> ⚠️ **KOREKSI (FASE 5 / T-15, 2026-09-22):** suite Jest yang disebut di bawah ("204 uji") TIDAK ADA lagi di repo (`frontend/src/**/*.test.*` = 0). Uji frontend saat ini = build CI (`yarn build`) saja; uji backend hermetik ada di `backend/tests/unit/` (`pytest tests/unit -n 0`), uji API di `tests/` (`test_fase*.py`), dan gate statis di `scripts/gate.sh`. Blok status di bawah adalah arsip sejarah.
>
...
## B. CURRENT STATUS (2026-05-25) — arsip
